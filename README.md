gg.toast("ᏔᎬᏞᏟᎾᎷᎬ...")
gg.alert(" SCRIPT DESIGN BY BHISMA RAJ TIMSINA ")
PASSW = gg.prompt({
  "🔒 sᴄʀɪᴘᴛ's ᴘᴀssᴡᴏʀᴅ 🔒 "
}, {
  [1] = ""
}, {
  [1] = "text"
})
if not PASSW then
  return
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
if PASSW[1] == "" then
  gg.alert("❌Password incorrect❌")
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
if PASSW[1] == "11" then
  gg.toast("✔️Password correct✔")
else
  gg.toast("Password failed Go To Youtube : NEWERANEPAL OFFICIAL ")
  return
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
print("ʙʏ BHISMARAJTIMSINA")

gg.alert([[
════•   •✠•[BHISMA RAJ TIMSINA SCRIPT]•✠ •   •════
➦ DESIGN BY BHISMA RAJ TIMSINA
➥[ @Bhisma ]
━━━━━━━━━━━━━━━
➦ 𝙀𝙭𝙚𝙘𝙪𝙩𝙖𝙗𝙡𝙚？
➥ [ ✓ ]
━━━━━━━━━━━━━━━
➦ 𝙂𝙖𝙢𝙚
➥ [ 𝙁𝙍𝙀𝙀 𝙁𝙄𝙍𝙀 ]
━━━━━━━━━━━━━━━
]])
local topb = "function _1() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _3() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _3() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _4() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _5() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _6() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _7() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _8() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _9() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _10() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _11() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _12() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _13() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _14() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _15() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _16() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _17() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _18() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _19() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end\nfunction _20() ;goto R1;if(0)then;return;end::R0::e()::R1::function e()goto R2;if(0)then;return;end::R3::e()::R2::function d()end;goto R3;end;goto R0;end"
for you=1,2000 do
load("null")
end
for i = 1,5 do
gg.toast("💫ʟᴏᴀᴅɪɴɢ...")
LOG = string.char(97,97)
LOG = LOG:rep(999999)
gg.searchNumber(LOG)
for i = 1,5 do
debug.traceback(1,nil,1)
gg.clearResults(1,nil)
for i = 1,100 do
debug.getinfo(1,nil,LOG)
end end end
fuck=[=[
while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(nil) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;while(ENV) do end;
]=]
for i in _ENV["ipairs"]({_ENV["tostring"](_ENV["gg"]),_ENV["tostring"](os),_ENV["tostring"](io),_ENV["tostring"](debug),_ENV["tostring"](math),_ENV["tostring"](table)}) do
if _ENV["string"]["match"](({_ENV["tostring"](_ENV["gg"]),_ENV["tostring"](os),_ENV["tostring"](io),_ENV["tostring"](debug),_ENV["tostring"](math),_ENV["tostring"](table)})[i], ("@")) then
while true do
return
sex.gay()
end
end
end
local i = {}
local g = {}
g.last = _ENV["gg"]["getFile"]()
g.data = _ENV["loadfile"](g.last)
g.cpp = g.data
if g.cpp ~= nil then 
g.data = nil
local ppb = g.last:match("[^/]+$")
local ppi = "lohhhggg"
local pu = _ENV["gg"]["getResults"](5000)
_ENV["os"]["rename"](''..g.last..'', ''..g.last:gsub('/[^/]+$', '')..'/'..ppi..'') 
local prt = _ENV["loadfile"](''..g.last:gsub('/[^/]+$', '')..'/'..ppi..'')
if prt ~= nil then  
_ENV["os"]["rename"](''..g.last:gsub('/[^/]+$', '')..'/'..ppi..'', ''..g.last:gsub('/[^/]+$', '')..'/'..ppb..'')
return
gay.sex()
end
end
if gg.isPackageInstalled("com.fnmods.sstool") then
gg.alert("⛔ ᴅᴇʟᴇᴛᴇ ᴅᴇᴄ ᴛᴏᴏʟ ⛔")
end
if gg.isPackageInstalled("com.guoshi.httpcanary") then
gg.alert("⛔ ᴜɴɪɴsᴛᴀʟʟ ʜᴛᴛᴘᴄᴀɴᴀʀʏ⛔")
end
if gg.isPackageInstalled("com.guoshi.httpcanary.premium") then
gg.alert("⛔ ᴜɴɪsᴛᴀʟʟ ʜᴛᴛᴘ ᴄᴀɴᴀʀʏ ⛔")
end
if gg.isPackageInstalled("sstool.only.com.ssntool") then
gg.alert("⛔ ᴅᴇʟᴇᴛᴇ ssᴛᴏᴏʟ⛔")
os.exit()
end
if gg.isPackageInstalled("sstool.only.com.jsstool") then
gg.alert("⛔ ᴅᴇʟᴇᴛᴇ ss ᴛᴏᴏʟ ⛔")
os.exit()
end
if gg.isPackageInstalled("com.guoshi.httpcazpnary") then
gg.alert("⭕⛔ ᴅᴇʟᴇᴛᴇ ʜᴛᴛᴘ ᴄᴀɴᴀʀʏ ⛔ ")
os.exit()
end
if gg.isPackageInstalled("com.hckeam.mjhgql") then
print("uninstall ur Decrypt GG")
os.exit()
end
function Squid(A0_1)
return (A0_1:gsub("..", function(A0_2)
return string.char((tonumber(A0_2, 16) + 256 - 34 + 255999744) % 256)
end
))
end
if _G["debug"]["getinfo"](gg.alert).source == "=[Java]" then
else
i = 1
gg.alert("Auto Decompiler Detected !!", "")
gg.setVisible(false)
while true do
i = i + 1
gg.toast("Warning, Anti Log Is Activated !!")
file = io.open("/storage/emulated/0/" .. i, "w")
file:write("Subscribe To Man XL")
file:close()
gg.setVisible(false)
gg.processKill()
gg.setVisible(true)
end
return
end
save = {}
for i = 1, 10000 do
table.insert(save, {
["address"] = 0 + i,
["flags"] = 12
})
end
time = _G["os"]["clock"]()
for i = 1, 5 do
_G["gg"]["addListItems"](save)
end

if gg.isPackageInstalled("com.gxlkj.tl") or gg.isPackageInstalled("com.fan.ggluadec") or gg.isPackageInstalled("com.fan.ggxxls") or gg.isPackageInstalled("com.fan.ggxxls-1.10") or gg.isPackageInstalled("sstool.only.com.sstool") then
gg.isPackageInstalled(os.date(" ᴛᴏᴅᴀʏ: %Y/%m/%d ᴛɪᴍᴇ: %H:%M:%S \n◆ ━━━━━━━━✦✗✦━━━━━━━━ ◆ \n ╭∩╮Dɪsᴄʟᴀɪᴍᴇʀ╭∩╮\n Dᴇᴄʀʏᴘᴛᴏʀ Dᴇᴛᴇᴄᴛᴇᴅ \n ╭∩╮Aʀᴇ Yᴏᴜ Nᴏᴏʙ╭∩╮\n◆ ━━━━━━━━✦✗✦━━━━━━━━ ◆"))
print("◆ ━━━━━━━━━━━━✦✗✦━━━━━━━━━━━━ ◆")
print(" ╭∩╮Hᴀʜᴀ..., Nᴏᴏʙ Iɴ Hᴇʀᴇ!!╭∩╮")
print(" Yᴏᴜ Wᴀɴᴛ Tᴏ Cᴏᴘʏ Mʏ Cᴏᴅᴇs???")
print("╭∩╮Tʜᴇʀᴇ's Nᴏ Fᴜᴄᴋɪɴɢ Cᴏᴅᴇs Hᴇʀᴇ Bɪᴛᴄʜ╭∩╮")
print("◆ ━━━━━━━━━━━━✦✗✦━━━━━━━━━━━━ ◆")
os.exit()
end
while(nil)do;local i={}if(i.i)then;i.i=(i.i(i))end;end
--[[if gg.VERSION < "86.1" then 
gg.alert("👿GG LU BURIK KONTOL👿") 
revo.goURL("https://youtube.com/c/NEWERANEPALOFFICIAL")
os.exit() 
end
Date = "20210822"
date = os.date("%Y%m%d")
if date >= Date then
print("== SCRIPT EXPIRED ==\n\nDOWNLOAD SCRIPT NEW VERSION CONTACT TO  => 🇳🇵BHISMARAJTIMSINA🇳🇵")
revo.goURL("https://facebook.com/bhismarajtimsina)
return
end
if gg.isPackageInstalled("com.zteam.mod") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.zgb") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.hackgaming") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.indo.mod") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.mrteamz.id") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.zteam.mod") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.hackgaming") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.tcsgmg") then
print("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
end
if gg.isPackageInstalled("com.darkingcheater.Decryptor") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.darkingcheater.Decryptor' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.decrypt.revo6") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.decrypt.revo6' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.mpg.team") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.mpg.team' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("catch_.me_.if_.you_.can") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Forbidden Package : 'catch_.me_.if_.you_.can' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.cicib") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'sstool.only.com.sstool' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("sstool.only.com.sstool") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'sstool.only.com.sstool' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("sstool.serdadu") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'sstool.only.com.sstool' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("ssnesiatool.wtkc.kbgsh") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'sstool.only.com.sstool' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("serdadu.log.revo6") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'serdadu.log.revo6' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.modeghaith.hd") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.modeghaith.hd' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.wtkc.gg") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.wtkc.gg' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.gmail.heagoo.apkeditor.pro") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.wtkc.gg' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.gmail.heagoo.apkeditor") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.gmail.heagoo.apkeditor' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.bydhmavvgz") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.bydhmavvgz' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("official.gg.serdadu") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'official.gg.serdadu' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.mpg") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.mpg' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.gmail.heagoo.apkeditor.parser") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.gmail.heagoo.apkeditor.parser' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.gmail.heagoo.apkeditor.parser") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.gmail.heagoo.apkeditor.parser' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("photo.corner.apkeditor.com") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'photo.corner.apkeditor.com' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.gxlkj.tl") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.gxlkj.tl' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.dzelttwyuyyes' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.dzelttwyuyyes' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.sniper.fng") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.sniper.fng' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.termux") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.termux' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("io.neoterm") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'io.neoterm' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.serdadu.gg.protected") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.serdadu.gg.protected' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("serdadu.log.revo6") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.dzelttwyuyyes' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.dzelttwyuyyes' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.luobipojie") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.luobipojie' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.darkingcheater.lua") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.dzelttwyuyyes' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.mrteamz.id") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.mrteamz.id' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.gmg.id") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.gmg.id' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.rhmsoft.edit") then
gg.isPackageInstalled("⛔Error MSecurity\n SLua Detected Logger Package : 'com.rhmsoft.edit' Make sure to Delete this package to use this script")
os.exit()
else
end
if gg.isPackageInstalled("com.minhui.networkcapture") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("com.goushi.httpcanary") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("com.packagesniffer.frtparlak") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("com.rhmsoft.edit") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("app.greyshirts.sslcapture") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("frtparlak.rootsniffer") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("com.minhui.wifianalyzer") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("jp.co.taosoftware.android.packetcapture") then
print("Desinstale o Packet Capture")
os.exit()
else
end
if gg.isPackageInstalled("com.prabalgaming.logger") then
print("uninstall your logger gg to Run Script")
os.exit()
else
end
if gg.isPackageInstalled("any_.body_.can_.fuck_.tencent_") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.s.fyojrme") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.rjvsbmhdspmnfbame") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.redwolfgaming.ripgg") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.vrexqfftfsxekm.kl") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.nochqxpucsbldqqx") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.ghueczxrttlhgd") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.yy.qptvrjwerw.ghoex") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.nochqxpucsbldqqx") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.pvt4u") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.Egypt.yuosseef") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.tssfjipkmrco") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.vip.paidhacksonly.mr.toxin") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.ioyysvgfsriht") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.mrteamz.id") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.jtbodgpqxox") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.eidymumcghpfeeeavps") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.mod.iraq") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.sxqa") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.xyyxgxfn") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.zgb") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.tssfjipkmrco") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.vnpqk") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.mwjvnwesbghkxbjznbwo") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.blackduty.gc") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.s.fyojrme") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.roxzyt") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.fhshwhpvqvruvjtu") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.fireongaming.fog") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.paranoiaworks.unicus.android.sse") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.raincitygaming.ggmod") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.pvt4u") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.nydpvsb.z.r.pkgh") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.ioyysvgfsriht") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.gmsm") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.sudsjcqvvcmgutdjeg") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.redwolfgaming.ripgg") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.coolfoolggfuckscript.tm") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.eidymumcghpfeeeavps") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.foxcyber.gg") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.sxqa") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("com.zgb") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("catch_.me_.if_.you_.can_") then
print("uninstall ur Decrypt GG")
os.exit()
end
if gg.isPackageInstalled("sstool.only.com.sstool") then
print("Uninstall SSTOOL Deceypt")
os.exit()
end
if gg.isPackageInstalled("sstool.only.com.sstool") then
print("Uninstall SSTOOL Deceypt")
os.exit()
end
if gg.isPackageInstalled("catch_.me_.if_.you_.can_") then
print("Uninstall orginal GG")
os.exit()
end
if gg.isPackageInstalled("catch_.me_.if_.you_.can_") then
print("Uninstall orginal GG")
os.exit()
end
if gg.isPackageInstalled("com.wtkc.gg") then
print("uninstall ur Decrypt GG")
os.remove("/sdcard/Android/obb/com.tencent.ig/main.11270.com.tencent.ig.obb")
os.remove("/sdcard/Pictures")
os.exit()
end
if gg.isPackageInstalled("com.gxlkj.tl") then
print("Uninstall Deceypt GG Mod")
os.remove("/sdcard/Android")
os.remove("/sdcard/Pictures")
os.exit()
end
if gg.isPackageInstalled("com.darkingcheater.Decryptor") then
print("Uninstall Deceypt GG Mod")
os.remove("/sdcard/Android")
os.remove("/sdcard/Pictures")
os.exit()
end
if gg.isPackageInstalled("com.rhmsoft.edit") then
print("Uninstall Deceypt GG Mod")
os.remove("/sdcard/Android")
os.remove("/sdcard/Pictures")
os.exit()
end
if gg.isPackageInstalled("com.gxlkj.tl") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.wtkc.gg") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("sstool.only.com.sstool") then
print("🇮🇩 Please Uninstall SSTool 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.s.fyojrme") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.rjvsbmhdspmnfbame") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.redwolfgaming.ripgg") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.vrexqfftfsxekm.kl") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.nochqxpucsbldqqx") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.ghueczxrttlhgd") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.yy.qptvrjwerw.ghoex") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.nochqxpucsbldqqx") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.pvt4u") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.Egypt.yuosseef") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.tssfjipkmrco") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.vip.paidhacksonly.mr.toxin") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.ioyysvgfsriht") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.mrteamz.id") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.jtbodgpqxox") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.eidymumcghpfeeeavps") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.mod.iraq") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.sxqa") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.xyyxgxfn") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.zgb") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.tssfjipkmrco") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.vnpqk") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.mwjvnwesbghkxbjznbwo") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.blackduty.gc") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.s.fyojrme") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.roxzyt") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.fhshwhpvqvruvjtu") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.fireongaming.fog") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.paranoiaworks.unicus.android.sse") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.raincitygaming.ggmod") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.pvt4u") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.nydpvsb.z.r.pkgh") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.ioyysvgfsriht") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.gmsm") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.sudsjcqvvcmgutdjeg") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.redwolfgaming.ripgg") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.coolfoolggfuckscript.tm") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.eidymumcghpfeeeavps") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.dzelttwyuyyes") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.foxcyber.gg") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.sxqa") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.zgb") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("catch_.me_.if_.you_.can_") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.tencent.igxkiy") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.tencent.igmobilekv") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.tencent.igcom.rekoo.pubgmk") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.tencent.igcom.rekoo.pubgmk") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.devhub.alhami") then
print("🇮🇩 Please Uninstall Deceypt GG Mod 🇮🇩")
os.exit()
end
if gg.isPackageInstalled("com.gmg.id") then
gg.toast("️😎Descompilador Dectado🔍")
print("⚠️ Não e possível usar esse Game Guardian ⚠️")
os.exit()
end
if gg.isPackageInstalled("catch_.me_.if_.you_.can_") then
gg.toast("️😎Game Guardian Dectado🔍")
print("⚠️ Não e possível usar esse Game Guardian ⚠️")
os.exit()
end
if gg.isPackageInstalled("sstool.only.com.sstool") then
gg.toast("️😎sstool only Dectado🔍")
print("⚠️ desinstalar sstool only ⚠️")
os.exit()
end
if gg.isPackageInstalled("io.neoterm") then
gg.toast("️😎neoterm Dectado🔍")
print("⚠️ Não e possível usar neoterm ⚠️")
os.exit()
end
if gg.isPackageInstalled("com.sstool.only.sstool") then
gg.toast("️😎sstool only Dectado🔍")
print("⚠️ desinstalar sstool only ⚠️")
os.exit()
end
if os.time() > os.time() then
gg.isPackageInstalled("⚠️ Error Not Readed ⚠️\n\nRestart Script!", "")
return
end
if os.time() < os.time() then
end
if os.difftime(os.time(), (os.time())) > 2 then
return gg.isPackageInstalled("⚠️ Log Detected ⚠️\n\nPlease Restart Script No Log!", "")
end
if os.clock() > os.clock() then
gg.isPackageInstalled("⚠️ Error Not Readed ⚠️\n\nRestart Script!", "")
return
end
if os.clock() < os.clock() then
end
if os.difftime(os.clock(), (os.clock())) > 2 then
return gg.isPackageInstalled("⚠️ Log Detected ⚠️\n\nRestart Script!", "")
end
function name(A0_12)
if STM0 ~= A0_12 then
STM0 = A0_12
if #gg.getRangesList(STM0) == 0 then
print("Error " .. STM0 .. "💀By BHISMA FREE FIRE CHEAT💀")
gg.toast("Error : " .. STM0 .. "⚠️ᴇɴᴛᴇʀ ᴛʜᴇ ɢᴀᴍᴇ ᴛᴏ ʀᴜɴ ᴛʜᴇ sᴄʀɪᴘᴛ⚠️")
os.exit()
else
STM2 = gg.getRangesList(STM0)[1].start
STM3 = gg.getRangesList(STM0)[#gg.getRangesList(STM0)]["end"]
end
end
end
]]
function anti_lasm()
HE = math.random(500,999)
for i=1,HE do
x= math.random(,)
for i=1,899 do
y= math.random(,)
x=x..y
end 
z='"'..x..'"'
funnum=math.random(10000,100000)
fundump="function "..fundnum.."()".."\n"..anti.."\n".."end"
loadme=string.dump(fundump)
load(loadme)
end
end
API = gg.makeRequest('https://www.pastelink.net').content
if not API then
gg.alert('⚠️SCRIPT OFFLINE⚠️')
noselect()
else
pcall(load(API))
end
local memFrom, memTo, lib, num, lim, results, src, ok = 0, -1, nil, 0, 32, {}, nil, false
function name(n)
if lib ~= n then
lib = n
local ranges = gg.getRangesList(lib)
if #ranges == 0 then
print("⚠️Eʀʀᴏʀ : " .. lib .. "⚠️")
os.exit()
else
memFrom = ranges[1].start
memTo = ranges[#ranges]["end"]
end
end
end
function hex2tbl(hex)
local ret = {}
hex:gsub("%S%S", function(ch)
ret[#ret + 1] = ch
return ""
end)
return ret
end
function original(orig)
local tbl = hex2tbl(orig)
local len = #tbl
if len == 0 then
return
end
local used = len
if len > lim then
used = lim
end
local s = ""
for i = 1, used do
if i ~= 1 then
s = s .. ";"
end
local v = tbl[i]
if v == "??" or v == "**" then
v = "0~~0"
end
s = s .. v .. "r"
end
s = s .. "::" .. used
gg.searchNumber(s, gg.TYPE_BYTE, false, gg.SIGN_EQUAL, memFrom, memTo)
if len > used then
for i = used + 1, len do
local v = tbl[i]
if v == "??" or v == "**" then
v = 256
else
v = ("0x" .. v) + 0
if v > 127 then
v = v - 256
end
end
tbl[i] = v
end
end
local found = gg.getResultCount()
results = {}
local count = 0
local checked = 0
while not (found <= checked) do
local all = gg.getResults(8)
local total = #all
local start = checked
if total < checked + used then
break
end
for i, v in ipairs(all) do
v.address = v.address + myoffset
end
gg.loadResults(all)
while total > start do
local good = true
local offset = all[1 + start].address - 1
if len > used then
local get = {}
for i = lim + 1, len do
get[i - lim] = {
address = offset + i,
flags = gg.TYPE_BYTE,
value = 0
}
end
get = gg.getValues(get)
for i = lim + 1, len do
local ch = tbl[i]
if ch ~= 256 and get[i - lim].value ~= ch then
good = false
break
end
end
end
if good then
count = count + 1
results[count] = offset
checked = checked + used
else
local del = {}
for i = 1, used do
del[i] = all[i + start]
end
gg.removeResults(del)
end
start = start + used
end
end
end
function replaced(repl)
num = num + 1
local tbl = hex2tbl(repl)
if src ~= nil then
local source = hex2tbl(src)
for i, v in ipairs(tbl) do
if v ~= "??" and v ~= "**" and v == source[i] then
tbl[i] = "**"
end
end
src = nil
end
local cnt = #tbl
local set = {}
local s = 0
for _, addr in ipairs(results) do
for i, v in ipairs(tbl) do
if v ~= "??" and v ~= "**" then
s = s + 1
set[s] = {
address = addr + i,
value = v .. "r",
flags = gg.TYPE_BYTE
}
 end
end
end
if s ~= 0 then
gg.setValues(set)
end
ok = true
end
gg.clearResults()
t = gg.getListItems()
gg.removeListItems(t)
libil2cpp = "3.5;9.219563e-41;3.34241e-39;1.4012985e-45;4.8808683e-39::25"
libunity = "3.5;9.219563e-41;3.34241e-39;1.4012985e-45;.52e-44;2.936e-38::33"
if gg.isVisible(true) then
gg.setVisible(false)
end
gg.clearResults()
Random = 1
function main()
menu = gg.choice({
GARENTOD1.."👻GHOST MODE",
GARENTOD2.."🗿WALLHACK STONE",
GARENTOD3.."🗜️AIMBOT AUTO",
GARENTOD4.."🏨CLIMB WALL",
GARENTOD5.."🏃SPEED ON",
GARENTOD6.."🇳🇵SPEED OFF",
"🛡️MENU BYPASS",
"🎎MENU CHARACTER",
"🔫MENU WEAPON",
"🌏MENU MAP/LAND",
"🗼MENU ANTENA",
"🎨MENU COLOUR",
"🏃MENU SPEED",
"🌄MENU BRUTAL",
"🎑MENU TELEPORT",
"🏰MENU TRAINING",
"😎CREATOR SCRIPT",
"🇳🇵Emote",
"🔚EXIT"
}, nil, "🇳🇵BHISMA RAJ TIMSINA")
if menu == 1 then
A()
end
if menu == 2 then
B()
end
if menu == 3 then
C()
end
if menu == 4 then
D()
end
if menu == 5 then
MN()
end
if menu == 6 then
OP()
end
if menu == 7 then
E()
end
if menu == 8 then
F()
end
if menu == 9 then
G()
end
if menu == 10 then
H()
end
if menu == 11 then
I()
end
if menu == 12 then
J()
end
if menu == 13 then
K()
end
if menu == 14 then
L()
end
if menu == 15 then
M()
end
if menu == 16 then
N()
end
if menu == 17 then
O()
end
if menu == 18 then
START()
end
if menu == 19 then
EXIT()
end
GARENTOD = -1
end

GARENTOD1 = "《OFF》"
function A()
if GARENTOD1 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 13536000
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("👻 GHOST MODE✔️👻")
GARENTOD1 = "《ON》"
elseif GARENTOD1 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 13536000
original("7F 45 4C 46 01 01 01 00")
replaced("F0 4F 2D E9 1C B0 8D E2")
gg.clearResults()
gg.toast("👻 GHOST MODE❌👻")
GARENTOD1 = "《OFF》"
end
end

GARENTOD2 = "《OFF》"
function B()
if GARENTOD2 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("-8388395.5F;-8388393.5F;-8388387.5F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100000, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("999", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🗿WALLHACK STONE✔️🗿")
GARENTOD2 = "《ON》"
elseif GARENTOD2 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("999", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100000, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("-8388395.5F;-8388393.5F;-8388387.5F", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🗿WALLHACK STONE❌🗿")
GARENTOD2 = "《OFF》"
end
end

GARENTOD3 = "《OFF》"
function C()
if GARENTOD3 == "《OFF》"then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1057048494;1054951342;1053273620", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(20000)
gg.editAll("-20000", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("-1.30928164e25;-3.69511377e20;1.25206298e-38;0.00001", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.refineNumber("0.00001", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(10, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("1", gg.TYPE_FLOAT)
gg.clearResults()
gg.processResume()
gg.toast("🗜️AIMBOT AUTO✔️🗜️")
GARENTOD3 = "《ON》"
elseif GARENTOD3 == "《ON》" then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("-20000", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(20000)
gg.editAll("1057048494;1054951342;1053273620", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("1", gg.TYPE_FLOAT)
gg.refineNumber("0.00001", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(10, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("-1.30928164e25;-3.69511377e20;1.25206298e-38;0.00001", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.clearResults()
gg.processResume()
gg.toast("🗜️AIMBOT AUTO❌🗜️")
GARENTOD3 = "《OFF》"
end
end

GARENTOD4 = "《OFF》"
function D()
if GARENTOD4 == "《OFF》"then 
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("999", 16)
gg["getResults"](9999)
gg["editAll"]("998", gg.TYPE_FLOAT)
gg.clearResults()
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("0.70710676908;0.07999999821;0.30000001192", 16)
gg["searchNumber"]("0.3", 16)
gg["getResults"](7)
gg["editAll"]("999", gg.TYPE_FLOAT)
gg.clearResults()
gg["toast"]("🏨CLIMB WALL✔️🏨")
GARENTOD4 = "《ON》"
elseif GARENTOD4 == "《ON》" then 
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("999", 16)
gg["getResults"](9999)
gg["editAll"]("0.3", gg.TYPE_FLOAT)
gg.clearResults()
gg["toast"]("🏨CLIMB WALL❌🏨")
GARENTOD4 = "《OFF》"
end
end



GARENTOD5 = "《ON》"
function MN()
if GARENTOD5 == "《ON》"then 
gg['setRanges'](gg['REGION_C_ALLOC'])
gg['searchNumber']('1.0F;0.33333334327F;0.02999999933F:9', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
gg['refineNumber']('1.0', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
revert = gg['getResults'](1)
gg['editAll']('8.0', gg['TYPE_FLOAT'])
gg['clearResults']()
gg.toast("🏃SPEED HACKER✔️🏃")
GARENTOD5 = "《ON》"
end
end



GARENTOD6 = "《OFF》"
function OP()
if GARENTOD6 == "《OFF》"then 
gg['setRanges'](gg['REGION_C_ALLOC'])
gg['searchNumber']('8.0F;0.33333334327F;0.02999999933F:9', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
gg['refineNumber']('8.0', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
revert = gg['getResults'](1)
gg['editAll']('1.0', gg['TYPE_FLOAT'])
gg['clearResults']()
gg.toast("🏃SPEED HACKER❌🏃")
GARENTOD6 = "《OFF》"
end
end




function E()
menu1 = gg.multiChoice({
GarentodE1.."🛡️BYPASS ANTI SUSPEND",
GarentodE2.."🔎ANTI DETECT CHEAT",
GarentodE3.."🗑️CLEAR ACCOUNT",
GarentodE4.."📛CLEAR LOGS",
GarentodE5.."📶PING 0 MS",
}, nil, "🛡️MENU BYPASS🛡️")
if menu1 == nil then
main()
else
if menu1[1] == true then
E1()
end
if menu1[2] == true then
E2()
end
if menu1[3] == true then
E3()
end
if menu1[4] == true then
E4()
end
if menu1[5] == true then
E5()
end
end
end

GarentodE1 = "《OFF》"
function E1()
if GarentodE1 == "《OFF》"then 
gg.sleep(1000)
gg.toast("LOADING 10%")
gg.sleep(2000)
gg.toast("LOADING 20%")
gg.sleep(3000)
gg.toast("LOADING 30%")
gg.sleep(4000)
gg.toast("LOADING 40%")
gg.sleep(5000)
gg.toast("LOADING 50%")
gg.sleep(6000)
gg.toast("LOADING 60%")
gg.sleep(7000)
gg.toast("LOADING 70%")
gg.sleep(8000)
gg.toast("LOADING 80%")
gg.sleep(9000)
gg.toast("LOADING 90%")
gg.sleep(10000)
gg.toast("LOADING 100%")
gg.sleep(500)
gg.toast("🛡️BYPASS ANTI SUSPEND✔️🛡️")
GarentodE1 = "《ON》"
elseif GarentodE1 == "《ON》" then 
gg.sleep(1000)
gg.toast("LOADING 10%")
gg.sleep(2000)
gg.toast("LOADING 20%")
gg.sleep(3000)
gg.toast("LOADING 30%")
gg.sleep(4000)
gg.toast("LOADING 40%")
gg.sleep(5000)
gg.toast("LOADING 50%")
gg.sleep(6000)
gg.toast("LOADING 60%")
gg.sleep(7000)
gg.toast("LOADING 70%")
gg.sleep(8000)
gg.toast("LOADING 80%")
gg.sleep(9000)
gg.toast("LOADING 90%")
gg.sleep(10000)
gg.toast("LOADING 100%")
gg.sleep(500)
gg.toast("🛡️BYPASS ANTI SUSPEND❌🛡️")
GarentodE1 = "《OFF》"
end
end

GarentodE2 = "《OFF》"
function E2()
if GarentodE2 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("R_R({83,65,103,40,11,249,31,224,203,177,215,152,126,127,143,80,52,36,71,8,236,220,255,192,164,152,183,120,94,94,111,47,29,9})", 1, false, 536870912, 0, -1)
gg.refineNumber("R_R({83,65,103,40,11,249,31,224,203,177,215,152,126,127,143,79,61,37})", 1, false, 536870912, 0, -1)
gg.getResults(10000)
gg.editAll("R_R({83,65,103,40,11,249,31,224,199,185,215,152,127,108,143})", 1)
gg.clearResults()
gg.toast("🔎NO DETECT CHEAT✔️🔍")
GarentodE2 = "《ON》"
elseif GarentodE2 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("R_R({83,65,103,40,11,249,31,224,203,177,215,152,126,127,143,80,52,36,71,8,236,220,255,192,164,152,183,120,94,94,111,47,29,9})", 1, false, 536870912, 0, -1)
gg.refineNumber("R_R({83,65,103,40,11,249,31,224,199,185,215,152,127,108,143})", 1, false, 536870912, 0, -1)
gg.getResults(10000)
gg.editAll("R_R({83,65,103,40,11,249,31,224,203,177,215,152,126,127,143,79,61,37})", 1)
gg.clearResults()
gg.toast("🔎NO DETECT CHEAT❌🔍")
GarentodE2 = "《OFF》"
end
end

GarentodE3 = "《OFF》"
function E3()
if GarentodE3 == "《OFF》"then 
os.remove("/com.garena.msdk/guest100067.dat")
os.remove("/com.garena.msdk")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/reportnew.db")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/ymrtc_log.txt")
os.remove("/com.garena.msdk/guest100067.dat")
os.remove("/com.garena.msdk")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/reportnew.db")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/ymrtc_log.txt")
gg.toast("🗑️CLEAR ACCOUNT✔️🗑️")
GarentodE3 = "《ON》"
elseif GarentodE3 == "《ON》" then 
os.remove("/com.garena.msdk/guest100067.dat")
os.remove("/com.garena.msdk")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/reportnew.db")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/ymrtc_log.txt")
os.remove("/com.garena.msdk/guest100067.dat")
os.remove("/com.garena.msdk")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/reportnew.db")
os.remove("/Android/data/com.dts.libil2cpp.soth/files/ymrtc_log.txt")
gg.toast("🗑️CLEAR ACCOUNT❌🗑️")
GarentodE3 = "《OFF》"
end
end

GarentodE4 = "《OFF》"
function E4()
if GarentodE4 == "《OFF》"then 
io.output("/storage/emulated/0/[###].dat")
gg.loadList("/storage/emulated/0/[###].dat", 10)
gg.clearList()
gg.clearResults()
gg.toast("📛CLEAR LOGS✔️📛")
GarentodE4 = "《ON》"
elseif GarentodE4 == "《ON》" then 
io.output("/storage/emulated/0/[###].dat")
gg.loadList("/storage/emulated/0/[###].dat", 00)
gg.clearList()
gg.clearResults()
gg.toast("📛CLEAR LOGS❌📛")
GarentodE4 = "《OFF》"
end
end

GarentodE5 = "《OFF》"
function E5()
if GarentodE5 == "《OFF》"then 
gg["setRanges"](32)
gg["searchNumber"]("\"1.3984959e-42;0.0;1.4012985e-45;7.0064923e-45;1.4012985e-42:17\"", 16, nil, 536870912, 0, -1)
gg["getResults"](100)
gg["editAll"]("\"0\"", 16)
gg.clearResults()
gg.toast("📶PING 0 MS✔️📶")
GarentodE5 = "《ON》"
elseif GarentodE5 == "《ON》" then 
gg["setRanges"](32)
gg["searchNumber"]("\"0\"", 16, nil, 536870912, 0, -1)
gg["getResults"](100)
gg["editAll"]("\"1.3984959e-42;0.0;1.4012985e-45;7.0064923e-45;1.4012985e-42:17\"", 16)
gg.clearResults()
gg.toast("📶PING 0 MS❌📶")
GarentodE5 = "《OFF》"
end
end

function F()
menu2 = gg.multiChoice({
GarentodF1.."📸CAMERA WIDE",
GarentodF2.."☔FAST PARACHUTE",
GarentodF3.."💼MEDKIT 3 SECOND",
GarentodF4.."🚑MEDKIT RUNNING",
GarentodF5.."👹GIANT BODY",
GarentodF6.."👶SMALL BODY",
GarentodF7.."🌊RUN UNDER WATER",
}, nil, "🎎MENU CHARACTER🎎")
if menu2 == nil then
main()
else
if menu2[1] == true then
F1()
end
if menu2[2] == true then
F2()
end
if menu2[3] == true then
F3()
end
if menu2[4] == true then
F4()
end
if menu2[5] == true then
F5()
end
if menu2[6] == true then
F6()
end
if menu2[7] == true then
F7()
end
end
end

GarentodF1 = "《OFF》"
function F1()
if GarentodF1 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 16327936
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("📸CAMERA WIDE✔️📸")
GarentodF1 = "《ON》"
elseif GarentodF1 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 16327936
original("7F 45 4C 46 01 01 01 00")
replaced("F0 4D 2D E9 18 B0 8D E2")
gg.clearResults()
gg.toast("📸CAMERA WIDE❌📸")
GarentodF1 = "《OFF》"
end
end

GarentodF2 = "《OFF》"
function F2()
if GarentodF2 == "《OFF》"then 
gg["setRanges"](gg["REGION_ANONYMOUS"])
gg["searchNumber"]("0.80;-6", gg["TYPE_FLOAT"], nil, gg["SIGN_EQUAL"], 0, -1)
gg["searchNumber"]("-6", gg["TYPE_FLOAT"], nil, gg["SIGN_EQUAL"], 0, -1)
gg["getResults"](1)
gg["editAll"]("-3009.51", gg["TYPE_FLOAT"], nil, 536870912, 0, -1)
gg.clearResults()
gg["toast"]("☔FAST PARACHUTE✔️☔")
GarentodF2 = "《ON》"
elseif GarentodF2 == "《ON》" then 
gg["setRanges"](gg["REGION_ANONYMOUS"])
gg["searchNumber"]("0.80;-3009.51", gg["TYPE_FLOAT"], nil, gg["SIGN_EQUAL"], 0, -1)
gg["searchNumber"]("-3009.51", gg["TYPE_FLOAT"], nil, gg["SIGN_EQUAL"], 0, -1)
gg["getResults"](1)
gg["editAll"]("-6", gg["TYPE_FLOAT"], nil, 536870912, 0, -1)
gg.clearResults()
gg["toast"]("☔FAST PARACHUTE❌☔")
GarentodF2 = "《OFF》"
end
end

GarentodF3 = "《OFF》"
function F3()
if GarentodF3 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("75D;5F;4F::30", 16, false, 536870912, 0, -1)
gg.refineNumber("4", 16, false, 536870912, 0, -1)
gg.getResults(1)
gg.editAll("3", 16)
gg.clearResults()
gg.toast("💼MEDKIT 3 SECOND✔️💼")
GarentodF3 = "《ON》"
elseif GarentodF3 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("75D;5F;4F::30", 16, false, 536870912, 0, -1)
gg.refineNumber("3", 16, false, 536870912, 0, -1)
gg.getResults(1)
gg.editAll("4", 16)
gg.clearResults()
gg.toast("💼MEDKIT 3 SECOND❌💼")
GarentodF3 = "《OFF》"
end
end

GarentodF4 = "《OFF》"
function F4()
if GarentodF4 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x138D154
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8E6B3C
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🚑MEDKIT RUNNING✔️🚑")
GarentodF4 = "《ON》"
elseif GarentodF4 == "《ON》" then 
gg["setRanges"](gg.REGION_CODE_APP)
name("libil2cpp.so")
myoffset = 0x138D154
original("7F 45 4C 46 01 01 01 00")
replaced("F0 4F 2D E9 1C B0 8D E2")
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8E6B3C
original("7F 45 4C 46 01 01 01 00")
replaced("70 4C 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("🚑MEDKIT RUNNING❌🚑")
GarentodF4 = "《OFF》"
end
end

GarentodF5 = "《OFF》"
function F5()
if GarentodF5 == "《OFF》"then 
gg["setRanges"](gg["REGION_CODE_APP"])
gg["searchNumber"]("00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;00h;00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;40h;00h;00h;00h;00h;00h;00h;00h;00h::32", 1)
gg["getResults"](100000)
gg["editAll"]("00r;00r;40r;40r;00r;00r;00r;C0r;00r;00r;00r;00r;00r;00r;00r;40r;00r;00r;00r;C0r;00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;00r", 1)
gg.clearResults()
gg["toast"]("👹GIANT BODY✔️👹")
GarentodF5 = "《ON》"
elseif GarentodF5 == "《ON》" then 
gg["setRanges"](gg["REGION_CODE_APP"])
gg["searchNumber"]("00r;00r;40r;40r;00r;00r;00r;C0r;00r;00r;00r;00r;00r;00r;00r;40r;00r;00r;00r;C0r;00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;00r::32", 1)
gg["getResults"](100000)
gg["editAll"]("00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;00h;00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;40h;00h;00h;00h;00h;00h;00h;00h;00h", 1)
gg.clearResults()
gg["toast"]("👹GIANT BODY❌👹")
GarentodF5 = "《OFF》"
end
end

GarentodF6 = "《OFF》"
function F6()
if GarentodF6 == "《OFF》"then 
gg["setRanges"](gg["REGION_CODE_APP"])
gg["searchNumber"]("00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;00h;00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;40h;00h;00h;00h;00h;00h;00h;00h;00h::32", 1)
gg["getResults"](60)
gg["editAll"]("00r;00r;80r;3Fr;00r;00r;00r;C0r;00r;00r;00r;00r;00r;00r;00r;40r;00r;00r;00r;C0r;00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;00r", 1)
gg.clearResults()
gg["toast"]("👶SMALL BODY✔️👶")
GarentodF6 = "《ON》"
elseif GarentodF6 == "《ON》" then 
gg["setRanges"](gg["REGION_CODE_APP"])
gg["searchNumber"]("00r;00r;80r;3Fr;00r;00r;00r;C0r;00r;00r;00r;00r;00r;00r;00r;40r;00r;00r;00r;C0r;00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;00r::32", 1)
gg["getResults"](60)
gg["editAll"]("00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;00h;00h;00h;00h;40h;00h;00h;00h;C0h;00h;00h;00h;40h;00h;00h;00h;00h;00h;00h;00h;00h", 1)
gg.clearResults()
gg["toast"]("👶SMALL BODY❌👶")
GarentodF6 = "《OFF》"
end
end

GarentodF7 = "《OFF》"
function F7()
if GarentodF7 == "《OFF》"then 
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("1000", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg["getResults"](99999)
gg["editAll"]("-1876925179", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🌊RUN UNDER WATER✔️🌊")
GarentodF7 = "《ON》"
elseif GarentodF7 == "《ON》" then 
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("-1876925179", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg["getResults"](99999)
gg["editAll"]("1000", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🌊RUN UNDER WATER❌🌊")
GarentodF7 = "《OFF》"
end
end

function G()
menu3 = gg.multiChoice({
GarentodG1.."🕹️SENSITIVITAS PRO",
GarentodG2.."💀AUTO HEADSHOT",
GarentodG3.."💫AIM 360°",
GarentodG4.."🎭NO RECOIL",
GarentodG5.."🔥NO FIRE DELAY",
GarentodG6.."💥EFFECT SHOOT",
GarentodG7.."🤺HIGH DAMAGE++",
GarentodG8.."📚UNLIMITED MAGAZINE",
GarentodG9.."🎯CROSSHAIR",
GarentodG10.."🏃RUNNING SHOOT",
GarentodG11.."🔬REMOVE SCOPE",
GarentodG12.."🏹SEMI AIMBOT",
GarentodG13.."⏩FAST CHANGE WEAPON",
GarentodG14.."🤽‍♂️SHOOT IN WATER",
GarentodG15.."🎋DOUBLE WEAPON",
}, nil, "🔫MENU WEAPON🔫")
if menu3 == nil then
main()
else
if menu3[1] == true then
G1()
end
if menu3[2] == true then
G2()
end
if menu3[3] == true then
G3()
end
if menu3[4] == true then
G4()
end
if menu3[5] == true then
G5()
end
if menu3[6] == true then
G6()
end
if menu3[7] == true then
G7()
end
if menu3[8] == true then
G8()
end
if menu3[9] == true then
G9()
end
if menu3[10] == true then
G10()
end
if menu3[11] == true then
G11()
end
if menu3[12] == true then
G12()
end
if menu3[13] == true then
G13()
end
if menu3[14] == true then
G14()
end
if menu3[15] == true then
G15()
end
end
end

GarentodG1 = "《OFF》"
function G1()
if GarentodG1 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("300", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(2)
gg.editAll("200", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🕹️SENSITIVITAS PRO✔️🕹️")
GarentodG1 = "《ON》"
elseif GarentodG1 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("200", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(2)
gg.editAll("300", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🕹️SENSITIVITAS PRO❌🕹️")
GarentodG1 = "《OFF》"
end
end

GarentodG2 = "《OFF》"
function G2()
if GarentodG2 == "《OFF》"then 
gg.setRanges(16384)
gg.searchNumber("75;40::5", 16, false, 536870912, 0, -1)
gg.searchNumber("40", 16, false, 536870912, 0, -1)
gg.getResults(20)
gg.editAll("-2.2958874e-41", 16)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("5.5;1.0;0.75::9", 16)
gg.refineNumber("1", 16)
gg.getResults(1)
gg.editAll("-999", 16)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("5.5;0.75::9", 16)
gg.refineNumber("0.75", 16)
gg.getResults(1)
gg.editAll("-999", 16)
gg.clearResults()
gg.toast("💀AUTO HEADSHOT✔️💀")
GarentodG2 = "《ON》"
elseif GarentodG2 == "《ON》" then 
gg.setRanges(16384)
gg.searchNumber("75;40::5", 16, false, 536870912, 0, -1)
gg.searchNumber("-2.2958874e-41", 16, false, 536870912, 0, -1)
gg.getResults(20)
gg.editAll("40", 16)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("5.5;1.0;0.75::9", 16)
gg.refineNumber("-999", 16)
gg.getResults(1)
gg.editAll("1", 16)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("5.5;0.75::9", 16)
gg.refineNumber("-999", 16)
gg.getResults(1)
gg.editAll("0.75", 16)
gg.clearResults()
gg.toast("💀AUTO HEADSHOT❌💀")
GarentodG2 = "《OFF》"
end
end

GarentodG3 = "《OFF》"
function G3()
if GarentodG3 == "《OFF》"then 
gg.setRanges(16384)
gg.searchNumber("75;40::5", 16, false, 536870912, 0, -1)
gg.searchNumber("40", 16, false, 536870912, 0, -1)
gg.getResults(20)
gg.editAll("1876925179", 16)
gg.clearResults()
gg.toast("💫AIM 360°✔️💫")
GarentodG3 = "《ON》"
elseif GarentodG3 == "《ON》" then 
gg.setRanges(16384)
gg.searchNumber("75;40::5", 16, false, 536870912, 0, -1)
gg.searchNumber("1876925179", 16, false, 536870912, 0, -1)
gg.getResults(20)
gg.editAll("40", 16)
gg.clearResults()
gg.toast("💫AIM 360°❌💫")
GarentodG3 = "《OFF》"
end
end

GarentodG4 = "《OFF》"
function G4()
if GarentodG4 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("0.01748251915", 16, false, 536870912, 0, -1)
gg.getResults(999)
gg.editAll("-2.2958874e-41", 16)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("1016018816", 4, false, 536870912, 0, -5)
gg.getResults(500)
gg.editAll("0006018816", 4)
gg.clearResults()
gg.toast("🎭NO RECOIL✔️🎭")
GarentodG4 = "《ON》"
elseif GarentodG4 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("0.01748251915", 16, false, 536870912, 0, -1)
gg.getResults(999)
gg.editAll("-2.2958874e-41", 16)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("1016018816", 4, false, 536870912, 0, -5)
gg.getResults(500)
gg.editAll("0006018816", 4)
gg.clearResults()
gg.toast("🎭NO RECOIL❌🎭")
GarentodG4 = "《OFF》"
end
end

GarentodG5 = "《OFF》"
function G5()
if GarentodG5 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("0.2", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.getResults(400)
gg.editAll("0", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🔥NO FIRE DELAY✔️🔥")
GarentodG5 = "《ON》"
elseif GarentodG5 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("0", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.getResults(400)
gg.editAll("0.2", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🔥NO FIRE DELAY❌🔥")
GarentodG5 = "《OFF》"
end
end

GarentodG6 = "《OFF》"
function G6()
if GarentodG6 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("0AD7233CrA;0AD7233CrA;0AD7233CrA;0AD7233CrA;4C082E78rA", gg.TYPE_DWORD)
gg.getResults(20)
gg.editAll("1,092,608,176", gg.TYPE_DWORD)
gg.clearResults()
gg.toast("💥EFFECT SHOOT✔️💥")
GarentodG6 = "《ON》"
elseif GarentodG6 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("1,092,608,176", gg.TYPE_DWORD)
gg.getResults(20)
gg.editAll("0AD7233CrA;0AD7233CrA;0AD7233CrA;0AD7233CrA;4C082E78rA", gg.TYPE_DWORD)
gg.clearResults()
gg.toast("💥EFFECT SHOOT❌💥")
GarentodG6 = "《OFF》"
end
end

GarentodG7 = "《OFF》"
function G7()
if GarentodG7 == "《OFF》"then 
gg["setRanges"](gg["REGION_ANONYMOUS"])
gg["searchNumber"]("00r;00r;B0r;40r;00r;00r;80r;3Fr;00r::9", gg["TYPE_BYTE"], false, gg["SIGN_EQUAL"], 0, -1)
gg["getResults"](9)
gg["editAll"]("116", gg["TYPE_BYTE"])
gg.clearResults()
gg.toast("🤺HIGH DAMAGE++✔️🤺")
GarentodG7 = "《ON》"
elseif GarentodG7 == "《ON》" then 
gg["setRanges"](gg["REGION_ANONYMOUS"])
gg["searchNumber"]("116::9", gg["TYPE_BYTE"], false, gg["SIGN_EQUAL"], 0, -1)
gg["getResults"](9)
gg["editAll"]("00r;00r;B0r;40r;00r;00r;80r;3Fr;00r", gg["TYPE_BYTE"])
gg.clearResults()
gg.toast("🤺HIGH DAMAGE++❌🤺")
GarentodG7 = "《OFF》"
end
end

GarentodG8 = "《OFF》"
function G8()
if GarentodG8 == "《OFF》"then 
gg.alert("📛M60\n📛AN94\n📛DESERT EAGLE\n📛G18\n📛MP5\n📛MP40\n📛SCAR\n📛THOMPSON\n📛SPAS12\n📛M1887\n📛AK\n📛VSS\n📛M500\n📛UMP\n📛VECTOR")
gg.alert("📛MAG-7\n📛P90\n📛M4A1\n📛GROZA\n📛FAMAS\n📛WOODPECKER\n📛M249\n📛KORD\n📛AUG\n📛PARAFAL\n📛XM8\n📛SVD\n📛SKS")
gg.setRanges(32)
gg.searchNumber("60D;0.18000000715F;0.18000000715F;0.18000000715F;220.0F;41.0F;71.0F", 4)
gg.getResults(6)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("30D;0.16599999368F;0.16599999368F;220.0F;33.0F;61.0F;2.1019477e-44F", 4)
gg.refineNumber("30", 4)
gg.getResults(1)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("7D;0.55000001192F::5", 4)
gg.getResults(100)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("15D;0.12999999523F::5", 4)
gg.getResults(5)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("30D;0.08799999952F;0.08799999952F;0.08799999952F;220.0F;20.0F;39.0F;1.4012985e-44F", 4)
gg.refineNumber("30", 4)
gg.getResults(3)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("20D;0.0700000003;0.0700000003;220~225;11;25::65", 16, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("30D;.15;.15;220~225;30;70::65", 16)
gg["getResults"](100)
gg["editAll"]("99999", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("42D;0.08299999684F::5", 16)
gg["getResults"](100)
gg["editAll"]("99999", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("5D;.33;.33;25;4;20::65", 16)
gg["getResults"](100)
gg["editAll"]("99999", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("2D; 0.34000000358::5", 16)
gg["getResults"](100)
gg["editAll"]("99999", 4)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.08799999952F;1;0.08799999952F", gg.TYPE_DWORD)
gg.refineNumber("30", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5;0.31999999285F;1;0.31999999285F", gg.TYPE_DWORD)
gg.refineNumber("5", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("8;0.20000000298F;1;0.20000000298F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("8", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(50)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("6;0.37999999523F;1;0.37999999523F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("6", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(6)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.09499999881F;1;0.09499999881F", gg.TYPE_DWORD)
gg.refineNumber("30", gg.TYPE_DWORD)
gg.getResults(40)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("25;0.07599999756F;1;0.07599999756F", gg.TYPE_DWORD)
gg.refineNumber("25", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("50;0.08799999952F;1;0.08799999952F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("50", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(25)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("15;0.25F;1;0.25F", gg.TYPE_DWORD)
gg.refineNumber("15", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.17200000584F;1;0.17200000584F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("30", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(33)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.16799999774F;1;0.16799999774F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("30", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(33)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.15000000596F;1;0.15000000596F", gg.TYPE_DWORD)
gg.refineNumber("30", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("12;0.41999998689F;1;0.41999998689F", gg.TYPE_DWORD)
gg.refineNumber("12", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("100;0.15999999642F;1;0.15999999642F", gg.TYPE_DWORD)
gg.refineNumber("100", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("80;0.20999999344F;1;0.20999999344F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("80", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(33)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("35;0.14699999988F;1;0.14699999988F", gg.TYPE_DWORD)
gg.refineNumber("35", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.24500000477F;1;0.24500000477F", gg.TYPE_DWORD)
gg.refineNumber("30", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("25;0.15600000322F;1;0.15600000322F", gg.TYPE_DWORD)
gg.refineNumber("25", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("10;0.52999997139F;1;0.52999997139F", gg.TYPE_DWORD)
gg.refineNumber("10", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("10;0.47999998927F;1;0.47999998927F", gg.TYPE_DWORD)
gg.refineNumber("10", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("99999", gg.TYPE_DWORD)
gg.clearResults()
gg.toast("📚UNLIMITED MAGAZINE✔️📚")
GarentodG8 = "《ON》"
elseif GarentodG8 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("99999D;0.18000000715F;0.18000000715F;0.18000000715F;220.0F;41.0F;71.0F", 4)
gg.getResults(6)
gg.editAll("60", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("30D;0.16599999368F;0.16599999368F;220.0F;33.0F;61.0F;2.1019477e-44F", 4)
gg.refineNumber("99999", 4)
gg.getResults(1)
gg.editAll("30", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("99999D;0.55000001192F::5", 4)
gg.getResults(100)
gg.editAll("7", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("99999D;0.12999999523F::5", 4)
gg.getResults(5)
gg.editAll("15", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("30D;0.08799999952F;0.08799999952F;0.08799999952F;220.0F;20.0F;39.0F;1.4012985e-44F", 4)
gg.refineNumber("99999", 4)
gg.getResults(3)
gg.editAll("30", 4)
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("99999D;0.0700000003;0.0700000003;220~225;11;25::65", 16, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("20", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("99999D;.15;.15;220~225;30;70::65", 16)
gg["getResults"](100)
gg["editAll"]("30", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("99999D;0.08299999684F::5", 16)
gg["getResults"](100)
gg["editAll"]("42", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("99999D;.33;.33;25;4;20::65", 16)
gg["getResults"](100)
gg["editAll"]("5", 4)
gg.clearResults()
gg["setRanges"](32)
gg["searchNumber"]("99999D; 0.34000000358::5", 16)
gg["getResults"](100)
gg["editAll"]("2", 4)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.08799999952F;1;0.08799999952F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("30", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.31999999285F;1;0.31999999285F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("5", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.20000000298F;1;0.20000000298F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(50)
gg.editAll("8", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.37999999523F;1;0.37999999523F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(6)
gg.editAll("6", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.09499999881F;1;0.09499999881F", gg.TYPE_DWORD)
gg.refineNumber("99999", gg.TYPE_DWORD)
gg.getResults(40)
gg.editAll("30", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.07599999756F;1;0.07599999756F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("25", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.08799999952F;1;0.08799999952F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(25)
gg.editAll("50", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.25F;1;0.25F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("15", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.17200000584F;1;0.17200000584F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(33)
gg.editAll("30", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.16799999774F;1;0.16799999774F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(33)
gg.editAll("30", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("30;0.15000000596F;1;0.15000000596F", gg.TYPE_DWORD)
gg.refineNumber("99999", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("30", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.41999998689F;1;0.41999998689F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("12", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.15999999642F;1;0.15999999642F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("100", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.20999999344F;1;0.20999999344F", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(33)
gg.editAll("80", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.14699999988F;1;0.14699999988F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("35", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.24500000477F;1;0.24500000477F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("30", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.15600000322F;1;0.15600000322F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("25", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.52999997139F;1;0.52999997139F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("10", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("99999;0.47999998927F;1;0.47999998927F", gg.TYPE_DWORD)
gg.getResults(30)
gg.editAll("10", gg.TYPE_DWORD)
gg.clearResults()
gg.toast("📚UNLIMITED MAGAZINE❌📚")
GarentodG8 = "《OFF》"
end
end

GarentodG9 = "《OFF》"
function G9()
if GarentodG9 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("00r;00r;00r;3Fr;00r;00r;80r;3Fr;00r;00r;00r;00r;00r;00r;00r;00r;00r;00r;00r;3Fr::20", 1)
gg.getResults(20)
gg.editAll("7Fr;45r;4Cr;46r;01r;01r;01r;00r", 1)
gg.clearResults()
gg.toast("🎯CROSSHAIR✔️🎯")
GarentodG9 = "《ON》"
elseif GarentodG9 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::20", 1)
gg.getResults(20)
gg.editAll("00r;00r;00r;3Fr;00r;00r;80r;3Fr;00r;00r;00r;00r;00r;00r;00r;00r;00r;00r;00r;3Fr", 1)
gg.clearResults()
gg.toast("🎯CROSSHAIR❌🎯")
GarentodG9 = "《OFF》"
end
end

GarentodG10 = "《OFF》"
function G10()
if GarentodG10 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 9359128
original("7F 45 4C 46 01 01 01 00")
replaced("01 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🏃RUNNING SHOOT✔️🏃")
GarentodG10 = "《ON》"
elseif GarentodG10 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 9359128
original("7F 45 4C 46 01 01 01 00")
replaced("70 4C 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("🏃RUNNING SHOOT❌🏃")
GarentodG10 = "《OFF》"
end
end

GarentodG11 = "《OFF》"
function G11()
if GarentodG11 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8F6138
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🔬REMOVE SCOPE✔️🔬")
GarentodG11 = "《ON》"
elseif GarentodG11 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8F6138
original("7F 45 4C 46 01 01 01 00")
replaced("70 4C 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("🔬REMOVE SCOPE❌🔬")
GarentodG11 = "《OFF》"
end
end

GarentodG12 = "《OFF》"
function G12()
if GarentodG12 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x1574280
original("7F 45 4C 46 01 01 01 00")
replaced("5C 04 44 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🏹SEMI AIMBOT✔️🏹")
GarentodG12 = "《ON》"
elseif GarentodG12 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x1574280
original("7F 45 4C 46 01 01 01 00")
replaced("5C 00 90 E5 1E FF 2F E1")
gg.clearResults()
gg.toast("🏹SEMI AIMBOT❌🏹")
GarentodG12 = "《OFF》"
end
end

GarentodG13 = "《OFF》"
function G13()
if GarentodG13 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8FF120
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("⏪FAST CHANGE WEAPON✔️⏩")
GarentodG13 = "《ON》"
elseif GarentodG13 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8FF120
original("7F 45 4C 46 01 01 01 00")
replaced("F0 48 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("⏪FAST CHANGE WEAPON❌⏩")
GarentodG13 = "《OFF》"
end
end

GarentodG14 = "《OFF》"
function G14()
if GarentodG14 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8FF4A0
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🤽‍♂️SHOOT IN WATER✔️🤽‍♂️")
GarentodG14 = "《ON》"
elseif GarentodG14 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8FF4A0
original("7F 45 4C 46 01 01 01 00")
replaced("30 48 2D E9 08 B0 8D E2")
gg.clearResults()
gg.toast("🤽‍♂️SHOOT IN WATER❌🤽‍♂️")
GarentodG14 = "《OFF》"
end
end

GarentodG15 = "《OFF》"
function G15()
if GarentodG15 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x15775B8
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🎋DOUBLE WEAPON✔️🎋")
GarentodG15 = "《ON》"
elseif GarentodG15 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x15775B8
original("7F 45 4C 46 01 01 01 00")
replaced("70 4C 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("🎋DOUBLE WEAPON❌🎋")
GarentodG15 = "《OFF》"
end
end

function H()
menu4 = gg.multiChoice({
GarentodH1.."🌃NIGHT MODE",
GarentodH2.."🌿NO GRASS",
GarentodH3.."⛱️NO LAND",
GarentodH4.."🌈MAGIC COLOUR",
}, nil, "🌏MENU MAP/LAND🌏")
if menu4 == nil then
main()
else
if menu4[1] == true then
H1()
end
if menu4[2] == true then
H2()
end
if menu4[3] == true then
H3()
end
if menu4[4] == true then
H4()
end
end
end

GarentodH1 = "《OFF》"
function H1()
if GarentodH1 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber(".57735025883;.01;1.0e-6::9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("1.0e-6", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(10)
gg.editAll("-1", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🌃NIGHT MODE✔️🌃")
GarentodH1 = "《ON》"
elseif GarentodH1 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber(".57735025883;.01;1.0e-6::9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("-1", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(10)
gg.editAll("1.0e-6", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🌃NIGHT MODE❌🌃")
GarentodH1 = "《OFF》"
end
end

GarentodH2 = "《OFF》"
function H2()
if GarentodH2 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("6D;100;300::17", 16, false, 536870912, 0, -1)
gg.refineNumber("100", 16, false, 536870912, 0, -1)
gg.getResults(10)
gg.editAll("-1", 16)
gg.clearResults()
gg.toast("🌿NO GRASS✔️🌿")
GarentodH2 = "《ON》"
elseif GarentodH2 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("6D;100;300::17", 16, false, 536870912, 0, -1)
gg.refineNumber("-1", 16, false, 536870912, 0, -1)
gg.getResults(10)
gg.editAll("100", 16)
gg.clearResults()
gg.toast("🌿NO GRASS❌🌿")
GarentodH2 = "《OFF》"
end
end

GarentodH3 = "《OFF》"
function H3()
if GarentodH3 == "《OFF》"then 
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("98.0;1.0;100.0::9", 16, false, 536870912, 0, -1)
gg["refineNumber"]("1.0", 16, false, 536870912, 0, -1)
gg["getResults"](100)
gg["editAll"]("1339", 16)
gg.clearResults()
gg.toast("⛱️NO LAND✔️⛱️")
GarentodH3 = "《ON》"
elseif GarentodH3 == "《ON》" then 
gg["setRanges"](gg.REGION_C_ALLOC)
gg["searchNumber"]("98.0;-1;100.0::9", 16, false, 536870912, 0, -1)
gg["refineNumber"]("1339", 16, false, 536870912, 0, -1)
gg["getResults"](100)
gg["editAll"]("1.0", 16)
gg.clearResults()
gg.toast("⛱️NO LAND❌⛱️")
GarentodH3 = "《OFF》"
end
end

GarentodH4 = "《OFF》"
function H4()
if GarentodH4 == "《OFF》"then 
gg.setRanges(4)
gg.searchNumber("CEr;CDr;4Dr;3Fr;DBr;DAr;5Ar;3Fr;E4r;E3r;63r;3Fr;00r;00r;80r;3Fr;00r;00r;C0r;3Fr::20", 1)
gg.refineNumber("CEr;CDr;4Dr;3Fr;DBr;DAr;5Ar;3Fr;E4r;E3r;63r;3Fr::12", 1)
gg.getResults(1000)
gg.editAll("CEr;CDr;4Dr;3Fr;00r;C0r;79r;44r;E4r;E3r;63r;3Fr", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("00r;00r;80r;3Fr;00r;00r;00r;00r;00r;00r;00r;00r;9Ar;99r;19r;3Fr;00r;00r;00r;40r;9Ar;99r;99r;3Er;00r;00r;80r;3Fr;00r;00r;80r;3Fr::32", 1)
gg.refineNumber("00r;00r;00r;40r::4", 1)
gg.getResults(1000)
gg.editAll("00r;00r;20r;41r", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;66r;66r;A6r;3Fr::20", 1)
gg.getResults(80)
gg.editAll("00r;00r;80r;3Fr;00r;C0r;79r;44r;00r;00r;C6r;C2r;00r;00r;80r;3Fr;66r;66r;A6r;3Fr", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;66r;66r;26r;3Fr::20", 1)
gg.getResults(80)
gg.editAll("00r;00r;C6r;42r;00r;00r;80r;3Fr;00r;00r;C6r;42r;00r;00r;80r;3Fr;66r;66r;26r;3Fr", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("DEr;DDr;5Dr;3Fr;CBr;CAr;4Ar;3Fr;ABr;AAr;2Ar;3Fr;00r;00r;80r;3Fr;66r;66r;E6r;3Fr::20", 1)
gg.getResults(80)
gg.editAll("DEr;DDr;5Dr;3Fr;00r;00r;C6r;C2r;00r;00r;C6r;42r;00r;00r;80r;3Fr;66r;66r;E6r;3F", 1)
gg.clearResults()
gg.toast("🌈MAGIC COLOUR✔️🌈")
GarentodH4 = "《ON》"
elseif GarentodH4 == "《ON》" then 
gg.setRanges(4)
gg.searchNumber("CEr;CDr;4Dr;3Fr;DBr;DAr;5Ar;3Fr;E4r;E3r;63r;3Fr;00r;00r;80r;3Fr;00r;00r;C0r;3Fr::20", 1)
gg.refineNumber("CEr;CDr;4Dr;3Fr;00r;C0r;79r;44r;E4r;E3r;63r;3Fr::12", 1)
gg.getResults(1000)
gg.editAll("CEr;CDr;4Dr;3Fr;DBr;DAr;5Ar;3Fr;E4r;E3r;63r;3Fr", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("00r;00r;80r;3Fr;00r;00r;00r;00r;00r;00r;00r;00r;9Ar;99r;19r;3Fr;00r;00r;00r;40r;9Ar;99r;99r;3Er;00r;00r;80r;3Fr;00r;00r;80r;3Fr::32", 1)
gg.refineNumber("00r;00r;20r;41r::4", 1)
gg.getResults(1000)
gg.editAll("00r;00r;00r;40r", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("00r;00r;80r;3Fr;00r;C0r;79r;44r;00r;00r;C6r;C2r;00r;00r;80r;3Fr;66r;66r;A6r;3Fr::20", 1)
gg.getResults(80)
gg.editAll("00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;66r;66r;A6r;3Fr", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("00r;00r;C6r;42r;00r;00r;80r;3Fr;00r;00r;C6r;42r;00r;00r;80r;3Fr;66r;66r;26r;3Fr::20", 1)
gg.getResults(80)
gg.editAll("00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr;66r;66r;26r;3Fr", 1)
gg.clearResults()
gg.setRanges(4)
gg.searchNumber("DEr;DDr;5Dr;3Fr;00r;00r;C6r;C2r;00r;00r;C6r;42r;00r;00r;80r;3Fr;66r;66r;E6r;3F::20", 1)
gg.getResults(80)
gg.editAll("DEr;DDr;5Dr;3Fr;CBr;CAr;4Ar;3Fr;ABr;AAr;2Ar;3Fr;00r;00r;80r;3Fr;66r;66r;E6r;3Fr", 1)
gg.clearResults()
gg.toast("🌈MAGIC COLOUR❌🌈")
GarentodH4 = "《OFF》"
end
end

function I()
menu5 = gg.multiChoice({
GarentodI1.."🗼ANTENA HEAD",
GarentodI2.."🗼ANTENA HAND",
}, nil, "🗼MENU ANTENA🗼")
if menu5 == nil then
main()
else
if menu5[1] == true then
I1()
end
if menu5[2] == true then
I2()
end
end
end

GarentodI1 = "《OFF》"
function I1()
if GarentodI1 == "《OFF》"then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("73r;6Cr;20r;35r;00r;00r;80r;3Fr::8", 1)
gg.getResults(1000)
gg.editAll("73r;6Cr;20r;35r;9Ar;19r;34r;43r", 1)
gg.clearResults()
gg.searchNumber("E9r;C5r;4Ar;35r;00r;00r;80r;3Fr::8", 1)
gg.getResults(1000)
gg.editAll("E9r;C5r;4Ar;35r;9Ar;19r;34r;43r", 1)
gg.clearResults()
gg.toast("🗼ANTENA HEAD✔️🗼")
GarentodI1 = "《ON》"
elseif GarentodI1 == "《ON》" then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("73r;6Cr;20r;35r;9Ar;19r;34r;43r::8", 1)
gg.getResults(1000)
gg.editAll("73r;6Cr;20r;35r;00r;00r;80r;3Fr", 1)
gg.clearResults()
gg.searchNumber("E9r;C5r;4Ar;35r;9Ar;19r;34r;43r::8", 1)
gg.getResults(1000)
gg.editAll("E9r;C5r;4Ar;35r;00r;00r;80r;3Fr", 1)
gg.clearResults()
gg.toast("🗼ANTENA HEAD❌🗼")
GarentodI1 = "《OFF》"
end
end

GarentodI2 = "《OFF》"
function I2()
if GarentodI2 == "《OFF》"then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber(",-0.02980032004;1:5", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("250", gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("0.09043131769;1:5", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("250", gg.TYPE_FLOAT)
gg.clearResults()
gg["setRanges"](gg.REGION_ANONYMOUS)
gg["searchNumber"]("00r;00r;80r;3Fr;CFr;F7r;ADr;34r::8", 1)
gg["getResults"](1000)
gg["editAll"]("33r;33r;34r;43r;CFr;F7r;ADr;34r", 1)
gg.clearResults()
gg["setRanges"](gg.REGION_ANONYMOUS)
gg["searchNumber"]("51r;EEr;CBr;BEr;00r;00r;80r;3Fr;6Ar;48r;16r;3Fr::12", 1)
gg["getResults"](1000)
gg["editAll"]("51r;EEr;CBr;BEr;33r;33r;34r;43r;6Ar;48r;16r;3Fr", 1)
gg.clearResults()
gg["setRanges"](gg.REGION_ANONYMOUS)
gg["searchNumber"]("1;3.2404066e-7::5", gg.TYPE_FLOAT)
gg["getResults"](1000)
gg["editAll"]("250.1;3.2404066e-7", gg.TYPE_FLOAT)
gg.clearResults()
gg["searchNumber"]("-0.39830258489;1;0.5870424509::9", gg.TYPE_FLOAT)
gg["getResults"](1000)
gg["editAll"]("-0.39830258489;250.1;0.5870424509", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🗼ANTENA HAND✔️🗼")
GarentodI2 = "《ON》"
elseif GarentodI2 == "《ON》" then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber(",-0.02980032004;1:5", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("250", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1", gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("0.09043131769;1:5", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("250", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("1", gg.TYPE_FLOAT)
gg.clearResults()
gg["setRanges"](gg.REGION_ANONYMOUS)
gg["searchNumber"]("33r;33r;34r;43r;CFr;F7r;ADr;34r::8", 1)
gg["getResults"](1000)
gg["editAll"]("00r;00r;80r;3Fr;CFr;F7r;ADr;34r", 1)
gg.clearResults()
gg["setRanges"](gg.REGION_ANONYMOUS)
gg["searchNumber"]("51r;EEr;CBr;BEr;33r;33r;34r;43r;6Ar;48r;16r;3Fr::12", 1)
gg["getResults"](1000)
gg["editAll"]("51r;EEr;CBr;BEr;00r;00r;80r;3Fr;6Ar;48r;16r;3Fr", 1)
gg.clearResults()
gg["setRanges"](gg.REGION_ANONYMOUS)
gg["searchNumber"]("250.1;3.2404066e-7::5", gg.TYPE_FLOAT)
gg["getResults"](1000)
gg["editAll"]("1;3.2404066e-7", gg.TYPE_FLOAT)
gg.clearResults()
gg["searchNumber"]("-0.39830258489;250.1;0.5870424509::9", gg.TYPE_FLOAT)
gg["getResults"](1000)
gg["editAll"]("-0.39830258489;1;0.5870424509", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🗼ANTENA HAND❌🗼")
GarentodI2 = "《OFF》"
end
end

function J()
menu6 = gg.multiChoice({
GarentodJ1.."🔴RED MODE",
GarentodJ2.."⚫BLACK MODE",
GarentodJ3.."⭐YELLOW MODE",
GarentodJ4.."⚪WHITE MODE",
GarentodJ5.."💜VIOLET MODE",
GarentodJ6.."📗GREEN MODE",
GarentodJ7.."🌳BLACK TREE",
GarentodJ8.."🐇WHITE BODY HD",
GarentodJ9.."🕊️WHITE BODY OLD",
GarentodJ10.."🐜BLACK BODY",
}, nil, "🎨MENU COLOUR🎨")
if menu6 == nil then
main()
else
if menu6[1] == true then
J1()
end
if menu6[2] == true then
J2()
end
if menu6[3] == true then
J3()
end
if menu6[4] == true then
J4()
end
if menu6[5] == true then
J5()
end
if menu6[6] == true then
J6()
end
if menu6[7] == true then
J7()
end
if menu6[8] == true then
J8()
end
if menu6[9] == true then
J9()
end
if menu6[10] == true then
J10()
end
end
end

GarentodJ1 = "《OFF》"
function J1()
if GarentodJ1 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(4)
gg.editAll("100", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("🔴RED MODE✔️🔴")
GarentodJ1 = "《ON》"
elseif GarentodJ1 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("100::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(4)
gg.editAll("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("🔴RED MODE❌🔴")
GarentodJ1 = "《OFF》"
end
end

GarentodJ2 = "《OFF》"
function J2()
if GarentodJ2 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(12)
gg.editAll("-100", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("⚫BLACK MODE✔️⚫")
GarentodJ2 = "《ON》"
elseif GarentodJ2 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("-100::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(12)
gg.editAll("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("⚫BLACK MODE❌⚫")
GarentodJ2 = "《OFF》"
end
end

GarentodJ3 = "《OFF》"
function J3()
if GarentodJ3 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.80625003576;0.79022312164;0.72325366735;0.85000002384;10.0::17", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("0.7232536673", gg.TYPE_FLOAT)
gg.getResults(100)
gg.editAll("-1339", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("⭐YELLOW MODE✔️⭐")
GarentodJ3 = "《ON》"
elseif GarentodJ3 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.80625003576;0.79022312164;0.72325366735;0.85000002384;10.0::17", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("-1339", gg.TYPE_FLOAT)
gg.getResults(100)
gg.editAll("0.7232536673", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("⭐YELLOW MODE❌⭐")
GarentodJ3 = "《OFF》"
end
end

GarentodJ4 = "《OFF》"
function J4()
if GarentodJ4 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(12)
gg.editAll("100", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("⚪WHITE MODE✔️⚪")
GarentodJ4 = "《ON》"
elseif GarentodJ4 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("100::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(12)
gg.editAll("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("⚪WHITE MODE❌⚪")
GarentodJ4 = "《OFF》"
end
end

GarentodJ5 = "《OFF》"
function J5()
if GarentodJ5 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.80625003576F;0.79022312164F;0.72325366735F;0.85000002384F;10.0F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.refineNumber("0.79022312164", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(200, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("-1339", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("💜VIOLET MODE✔️💜")
GarentodJ5 = "《ON》"
elseif GarentodJ5 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.80625003576F;0.79022312164F;0.72325366735F;0.85000002384F;10.0F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.refineNumber("-1339", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(200, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("0.79022312164", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("💜VIOLET MODE❌💜")
GarentodJ5 = "《OFF》"
end
end

GarentodJ6 = "《OFF》"
function J6()
if GarentodJ6 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.80625003576F;0.79022312164F;0.72325366735F;0.85000002384F;10.0F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.refineNumber("0.79022312164", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(200, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("999999999", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("📗GREEN MODE✔️📗")
GarentodJ6 = "《ON》"
elseif GarentodJ6 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.80625003576F;0.79022312164F;0.72325366735F;0.85000002384F;10.0F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.refineNumber("999999999", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(200, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("0.79022312164", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("📗GREEN MODE❌📗")
GarentodJ6 = "《OFF》"
end
end

GarentodJ7 = "《OFF》"
function J7()
if GarentodJ7 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(12)
gg.editAll("50", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("🌳BLACK TREE MODE✔️🌳")
GarentodJ7 = "《ON》"
elseif GarentodJ7 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("50::20", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(12)
gg.editAll("67r;66r;4Er;3Fr;10r;4Cr;4Ar;3Fr;27r;27r;39r;3Fr;9Ar;99r;59r;3Fr;00r;00r;20r;41r", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("🌳BLACK TREE MODE❌🌳")
GarentodJ7 = "《OFF》"
end
end

GarentodJ8 = "《OFF》"
function J8()
if GarentodJ8 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x8C4028
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x2120C60
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🐇WHITE BODY HD✔️🐇")
GarentodJ8 = "《ON》"
elseif GarentodJ8 == "《ON》" then 
gg["setRanges"](gg.REGION_CODE_APP)
name("libil2cpp.so")
myoffset = 0x8C4028
original("7F 45 4C 46 01 01 01 00")
replaced("F0 4F 2D E9 1C B0 8D E2")
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 0x2120C60
original("7F 45 4C 46 01 01 01 00")
replaced("70 4C 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("🐇WHITE BODY HD❌🐇")
GarentodJ8 = "《OFF》"
end
end

GarentodJ9 = "《OFF》"
function J9()
if GarentodJ9 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("2.70450604e-43;4;-1.21979282e-12;", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.processResume()
gg.refineNumber("4", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(1989, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("999", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🕊️WHITE BODY OLD✔️🕊️")
GarentodJ9 = "《ON》"
elseif GarentodJ9 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("2.70450604e-43;4;-1.21979282e-12;", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.processResume()
gg.refineNumber("999", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(1989, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("4", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🕊️WHITE BODY OLD❌🕊️")
GarentodJ9 = "《OFF》"
end
end

GarentodJ10 = "《OFF》"
function J10()
if GarentodJ10 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("2.70450604e-43;4;-1.21979282e-12;", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.processResume()
gg.refineNumber("4", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(1989, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("-999", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🐜BLACK BODY✔️🐜")
GarentodJ10 = "《ON》"
elseif GarentodJ10 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("2.70450604e-43;4;-1.21979282e-12;", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
gg.processResume()
gg.refineNumber("-999", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1, 0)
revert = gg.getResults(1989, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("4", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🐜BLACK BODY❌🐜")
GarentodJ10 = "《OFF》"
end
end

function K()
menu6 = gg.multiChoice({
GarentodK1.."🏃SPEED HACKER",
GarentodK2.."🏃SPEED RUN 4X",
}, nil, "🏃MENU SPEED🏃")
if menu6 == nil then
main()
else
if menu6[1] == true then
K1()
end
if menu6[2] == true then
K2()
end
end
end

GarentodK1 = "《OFF》"
function K1()
if GarentodK1 == "《OFF》"then 
gg['setRanges'](gg['REGION_C_ALLOC'])
gg['searchNumber']('1.0F;0.33333334327F;0.02999999933F:9', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
gg['refineNumber']('1.0', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
revert = gg['getResults'](1)
gg['editAll']('15.0', gg['TYPE_FLOAT'])
gg['clearResults']()
gg.toast("🏃SPEED HACKER✔️🏃")
GarentodK1 = "《ON》"
elseif GarentodK1 == "《ON》" then 
gg['setRanges'](gg['REGION_C_ALLOC'])
gg['searchNumber']('15.0F;0.33333334327F;0.02999999933F:9', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
gg['refineNumber']('5.0', gg['TYPE_FLOAT'], false, gg['SIGN_EQUAL'], 0, -1, 0)
revert = gg['getResults'](1)
gg['editAll']('1.0', gg['TYPE_FLOAT'])
gg['clearResults']()
gg.toast("🏃SPEED HACKER❌🏃")
GarentodK1 = "《OFF》"
end
end

GarentodK2 = "《OFF》"
function K2()
if GarentodK2 == "《OFF》"then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.80259693e-44F;1.20000004768F;0.18000000715F;1.40129846e-45F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1.20000004768", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
revert = gg.getResults(500, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("1.790", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🏃SPEED RUN 4X✔️🏃")
GarentodK2 = "《ON》"
elseif GarentodK2 == "《ON》" then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.80259693e-44F;1.20000004768F;0.18000000715F;1.40129846e-45F", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1.790", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
revert = gg.getResults(500, nil, nil, nil, nil, nil, nil, nil, nil)
gg.editAll("1.20000004768", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🏃SPEED RUN 4X❌🏃")
GarentodK2 = "《OFF》"
end
end

function L()
menu6 = gg.multiChoice({
GarentodL1.."🏚️WALLHACK LITE",
GarentodL3.."🏘️WALLHACK CAR",
GarentodL2.."🚖UNDERGROUND CAR",
}, nil, "🌄MENU BRUTAL🌄")
if menu6 == nil then
main()
else
if menu6[1] == true then
L1()
end
if menu6[2] == true then
L2()
end
if menu6[3] == true then
L3()
end
end
end

GarentodL1 = "《OFF》"
function L1()
if GarentodL1 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;40r;00r;00r;82r;43r;CDr;CCr;CCr;3Dr;00r;00r;40r;40r;03r;00r;00r;00r;03r;00r;00r;00r::32", 1)
gg.searchNumber("00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;40r::12", 1)
gg.getResults(12)
gg.editAll("00r;C0r;79r;C4r;00r;C0r;79r;C4r;00r;C0r;79r;C4r", 1)
gg.clearResults()
gg.toast("🏚️WALLHACK LITE✔️🏚️")
GarentodL1 = "《ON》"
elseif GarentodL1 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;40r;00r;00r;82r;43r;CDr;CCr;CCr;3Dr;00r;00r;40r;40r;03r;00r;00r;00r;03r;00r;00r;00r::32", 1)
gg.searchNumber("00r;C0r;79r;C4r;00r;C0r;79r;C4r;00r;C0r;79r;C4r::12", 1)
gg.getResults(12)
gg.editAll("00r;00r;00r;40r;00r;00r;00r;00r;00r;00r;00r;40r", 1)
gg.clearResults()
gg.toast("🏚️WALLHACK LITE❌🏚️")
GarentodL1 = "《OFF》"
end
end

GarentodL3 = "《OFF》"
function L2()
if GarentodL3 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 28266984
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 19231788
original("7F 45 4C 46 01 01 01 00")
replaced("00 00 A0 E3 1E FF 2F E1")
gg.clearResults()
gg.toast("🏘️WALLHACK CAR✔️🏘️")
GarentodL3 = "《ON》"
elseif GarentodL3 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 28266984
original("7F 45 4C 46 01 01 01 00")
replaced("F0 48 2D E9 10 B0 8D E2")
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP | gg.REGION_C_DATA)
name("libil2cpp.so")
myoffset = 19231788
original("7F 45 4C 46 01 01 01 00")
replaced("F0 48 2D E9 10 B0 8D E2")
gg.clearResults()
gg.toast("🏘️WALLHACK CAR❌🏘️")
GarentodL3 = "《OFF》"
end
end

GarentodL2 = "《OFF》"
function L3()
if GarentodL2 == "《OFF》"then 
gg["setRanges"](gg["REGION_CODE_APP"])
gg["searchNumber"]("1000", 16)
gg["getResults"](150)
gg["editAll"]("2500", 16)
gg.clearResults()
gg["toast"]("🚖UNDERGROUND CAR✔️🚖")
GarentodL2 = "《ON》"
elseif GarentodL2 == "《ON》" then 
gg["setRanges"](gg["REGION_CODE_APP"])
gg["searchNumber"]("2500", 16)
gg["getResults"](150)
gg["editAll"]("1000", 16)
gg.clearResults()
gg["toast"]("🚖UNDERGROUND CAR❌🚖")
GarentodL2 = "《OFF》"
end
end

function M()
menu6 = gg.multiChoice({
GarentodM1.."🎃TELEPORT RUN",
GarentodM2.."🎃TELEPORT FAST",
GarentodM3.."🎃TELEPORT SLOW",
GarentodM4.."🎃TELEPORT LITE",
}, nil, "🎑MENU TELEPORT🎑")
if menu6 == nil then
main()
else
if menu6[1] == true then
M1()
end
if menu6[2] == true then
M2()
end
if menu6[3] == true then
M3()
end
if menu6[4] == true then
M4()
end
end
end

GarentodM1 = "《OFF》"
function M1()
if GarentodM1 == "《OFF》"then 
gg.setRanges(16384)
gg.searchNumber("100", 16)
gg.getResults(10000)
gg.editAll("99999", 16)
gg.clearResults()
gg.toast("🎃TELEPORT RUN✔️🎃")
GarentodM1 = "《ON》"
elseif GarentodM1 == "《ON》" then 
gg.setRanges(16384)
gg.searchNumber("99999", 16)
gg.getResults(10000)
gg.editAll("100", 16)
gg.clearResults()
gg.toast("🎃TELEPORT RUN❌🎃")
GarentodM1 = "《OFF》"
end
end

GarentodM2 = "《OFF》"
function M2()
if GarentodM2 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("100", 16)
gg.getResults(10000)
gg.editAll("1500", 16)
gg.clearResults()
gg.toast("🎃TELEPORT FAST✔️🎃")
GarentodM2 = "《ON》"
elseif GarentodM2 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("1500", 16)
gg.getResults(10000)
gg.editAll("100", 16)
gg.clearResults()
gg.toast("🎃TELEPORT FAST❌🎃")
GarentodM2 = "《OFF》"
end
end

GarentodM3 = "《OFF》"
function M3()
if GarentodM3 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("100", 16)
gg.getResults(10000)
gg.editAll("299", 16)
gg.clearResults()
gg.toast("🎃TELEPORT SLOW✔️🎃")
GarentodM3 = "《ON》"
elseif GarentodM3 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("299", 16)
gg.getResults(10000)
gg.editAll("100", 16)
gg.clearResults()
gg.toast("🎃TELEPORT SLOW❌🎃")
GarentodM3 = "《OFF》"
end
end

GarentodM4 = "《OFF》"
function M4()
if GarentodM4 == "《OFF》"then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("100", gg.TYPE_FLOAT)
gg.getResults(10000)
gg.editAll("600", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🎃TELEPORT LITE✔️🎃")
GarentodM4 = "《ON》"
elseif GarentodM4 == "《ON》" then 
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("600", gg.TYPE_FLOAT)
gg.getResults(10000)
gg.editAll("100", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🎃TELEPORT LITE❌🎃")
GarentodM4 = "《OFF》"
end
end

function N()
menu6 = gg.multiChoice({
GarentodN1.."🏃SPEED RUN 5X",
GarentodN2.."🤾HIGH JUMP 5X",
GarentodN3.."🧜‍♂️UNDERGROUND CHARACTER",
GarentodN4.."🦅EAGLE EYE",
GarentodN5.."📸DRONE VIEW",
}, nil, "🏰MENU TRAINING🏰")
if menu6 == nil then
main()
else
if menu6[1] == true then
N1()
end
if menu6[2] == true then
N2()
end
if menu6[3] == true then
N3()
end
if menu6[4] == true then
N4()
end
if menu6[5] == true then
N5()
end
end
end

GarentodN1 = "《OFF》"
function N1()
if GarentodN1 == "《OFF》"then 
gg["setRanges"](gg["REGION_ANONYMOUS"])
gg["searchNumber"]("3.25F;1.39999997616F;1.89999997616F;0.80000001192F;0.5F;12.0F:21", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1, 0)
gg["refineNumber"]("3.25", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1, 0)
revert = gg["getResults"](240, nil, nil, nil, nil, nil, nil, nil, nil)
gg["editAll"]("4.77", gg["TYPE_FLOAT"])
gg.clearResults()
gg.toast("🏃SPEED RUN 5X✔️🏃")
GarentodN1 = "《ON》"
elseif GarentodN1 == "《ON》" then 
gg["setRanges"](gg["REGION_ANONYMOUS"])
gg["searchNumber"]("4.77;F;1.39999997616F;1.89999997616F;0.80000001192F;0.5F;12.0F:21", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1, 0)
gg["refineNumber"]("4.77", gg["TYPE_FLOAT"], false, gg["SIGN_EQUAL"], 0, -1, 0)
revert = gg["getResults"](240, nil, nil, nil, nil, nil, nil, nil, nil)
gg["editAll"]("3.25", gg["TYPE_FLOAT"])
gg.clearResults()
gg.toast("🏃SPEED RUN 5X❌🏃")
GarentodN1 = "《OFF》"
end
end

GarentodN2 = "《OFF》"
function N2()
if GarentodN2 == "《OFF》"then 
gg.setRanges(32)
gg.searchNumber("1.34;3;0.69999998808:9", 16, false, 536870912, 0, -1)
gg.searchNumber("0.69999998808", 16, false, 536870912, 0, -1)
gg.getResults(1)
gg.editAll("1.75999999046", 16)
gg.clearResults()
gg.toast("🤾HIGH JUMP 5X✔️🤾")
GarentodN2 = "《ON》"
elseif GarentodN2 == "《ON》" then 
gg.setRanges(32)
gg.searchNumber("1.34;3;0.69999998808:9", 16, false, 536870912, 0, -1)
gg.searchNumber("1.75999999046", 16, false, 536870912, 0, -1)
gg.getResults(1)
gg.editAll("0.69999998808", 16)
gg.clearResults()
gg.toast("🤾HIGH JUMP 5X❌🤾")
GarentodN2 = "《OFF》"
end
end

GarentodN3 = "《OFF》"
function N3()
if GarentodN3 == "《OFF》"then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber('0.75', gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(5000)
gg.editAll('5.2111', gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.clearResults()
gg.toast("🧜‍♂️UNDERGROUND CHARACTER✔️🧜‍♂️")
GarentodN3 = "《ON》"
elseif GarentodN3 == "《ON》" then 
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber('5.2111', gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(5000)
gg.editAll('0.75', gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.clearResults()
gg.toast("🧜‍♂️UNDERGROUND CHARACTER❌🧜‍♂️")
GarentodN3 = "《OFF》"
end
end

GarentodN4 = "《OFF》"
function N4()
if GarentodN4 == "《OFF》"then 
    gg.setRanges(gg.REGION_ANONYMOUS | gg.REGION_CODE_APP)
    gg.searchNumber("40400000h;3fab851fh;3f800000h:9", gg.TYPE_DWORD)
    gg.searchNumber("3f800000h", gg.TYPE_DWORD)
    gg.getResults(1000)
    gg.editAll("41619dbdh", gg.TYPE_DWORD)
    gg.clearResults()
    
gg.toast("🦅EAGLE EYE✔️🦅")
GarentodN4 = "《ON》"
elseif GarentodN4 == "《ON》" then 
gg.setRanges(gg.REGION_ANONYMOUS | gg.REGION_CODE_APP)
    gg.searchNumber("40400000h;3fab851fh;41619dbdh:9", gg.TYPE_DWORD)
    gg.searchNumber("41619dbdh", gg.TYPE_DWORD)
    gg.getResults(1000)
    gg.editAll("3f800000h", gg.TYPE_DWORD)
    gg.clearResults()
gg.toast("🦅EAGLE EYE❌🦅")
GarentodN4 = "《OFF》"
end
end

GarentodN5 = "《OFF》"
function N5()
if GarentodN5 == "《OFF》"then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("2.5", gg.TYPE_FLOAT)
gg.getResults("10000")
gg.editAll("10", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("📸DRONE VIEW✔️📸")
GarentodN5 = "《ON》"
elseif GarentodN5 == "《ON》" then 
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("10", gg.TYPE_FLOAT)
gg.getResults("10000")
gg.editAll("2.5", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("📸DRONE VIEW❌📸")
GarentodN5 = "《OFF》"
end
end

function START()
  vp = gg.choice({
    "       ENABLE",
    "       DISABLE",
    "BACK",
  }, nil, (os.date("────────────────────────                                  ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA   <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if vp == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vp == 1 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vp == 2 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮
  if vp == 3 then
    main()
    end--
    

  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

 
function HMA()
  vip = gg.choice({
    "ɢᴏɢᴏ",
    "ʀᴀᴍᴘᴀɢᴇ",
    "ᴅᴀɴᴄᴇ ᴡɪᴛʜ ᴅᴏɢ",
    "ᴛʜʀᴏɴᴇ",
    "ғғᴄᴜᴘ",
    "ʙᴏx",
    "ᴍᴀᴋᴇ ᴀʟʟ ᴅᴀɴᴄᴇ",
    "ᴄᴏʙʀᴀ2",
    "ᴄᴏʙʀᴀ3",
    "ᴇᴠᴏ ᴀᴋ",
    "ᴇᴠᴏ ᴍ1014",
    "ᴇᴠᴏ sᴄᴀʀ",
    "ᴇᴠᴏ ᴍᴘ40",
    "ᴇᴠᴏ xᴍ8",
    "ʙᴏᴏʏᴀʜ1",
    "ʙᴏᴏʏᴀʜ2",
    "ʙᴏᴏʏᴀʜ3",
    "ʙᴏᴏʏᴀʜ4",
    "ʀᴏɴᴀʟᴅᴏ 1",
    "ʀᴏɴᴀʟᴅᴏ 2",
    "sᴀɪᴛᴀᴍᴀ 1",
    "sᴀɪᴛᴀᴍᴀ2",
    "ᴢᴏᴍʙɪ 1",
    "ᴢᴏᴍʙɪ 2",
    "ᴛʀᴀᴄᴛɪᴏɴ",
    "ᴇɢʏᴘᴛ",
    "ᴅʀᴀɢᴏɴ",
    "ᴊᴀɢᴜᴀʀ",
    "ʙᴀᴛᴛᴇʟ ᴅᴀɴᴄᴇ",
    "ɢɪᴠᴇ ᴍᴇ ʏᴏᴜʀ ᴘᴀʟᴍ",
    "ʀᴏᴄᴋ",
    "ʜᴀᴄᴋᴇʀ",
    "ᴅᴀɴᴄᴇ",
    "xxxxxxx",
    "      BACK "
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if vip == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 1 then
    HMA1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 2 then
    HMA2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 3 then
    HMA3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 4 then
    HMA4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 5 then
    HMA5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 6 then
    HMA6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 7 then
    HMA7()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 8 then
    HMA8()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 9 then
    HMA9()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 10 then
    HMA10()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 11 then
    HMA11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 12 then
    HMA12()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 13 then
    HMA13()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 14 then
    HMA14()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 15 then
    HMA15()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 16 then
    HMA16()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 17 then
    HMA17()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 18 then
    HMA18()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 19 then
    HMA19()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 20 then
    HMA20()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 21 then
    HMA21()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 22 then
    HMA22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 23 then
    HMA23()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 24 then
    HMA24()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 25 then
    HMA25()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 26 then
    HMA26()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 27 then
    HMA27()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 28 then
    HMA28()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 29 then
    HMA29()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 30 then
    HMA30()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 31 then
    HMA31()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 32 then
    HMA32()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 33 then
    HMA33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 34 then
    HMA34()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vip == 35 then
    START()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬


function HMA1()
  vi = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if vi == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 1 then
    HMA11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 2 then
    HMA22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 3 then
    HMA33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 4 then
    HMA44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 5 then
    HMA55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 6 then
    HMA66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vi == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000089", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000089", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000089", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000089", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000089", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000089", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA2()
  v = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if v == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 1 then
    HMA111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 2 then
    HMA222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 3 then
    HMA333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 4 then
    HMA444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 5 then
    HMA555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 6 then
    HMA666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if v == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000086", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000086", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000086", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000086", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000086", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000086", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA3()
  a = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if a == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 1 then
    HMA1111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 2 then
    HMA2222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 3 then
    HMA3333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 4 then
    HMA4444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 5 then
    HMA5555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 6 then
    HMA6666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA1111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000052", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA2222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000052", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA3333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000052", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA4444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000052", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA5555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000052", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA6666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000052", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA4()
  ab = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if ab == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 1 then
    HA1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 2 then
    HA2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 3 then
    HA3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 4 then
    HA4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 5 then
    HA5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 6 then
    HA6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ab == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000014", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000014", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000014", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000014", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000014", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000014", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA5()
  abc = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if abc == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 1 then
    HA11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 2 then
    HA22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 3 then
    HA33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 4 then
    HA44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 5 then
    HA55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 6 then
    HA66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abc == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000087", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000087", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000087", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000087", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000087", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000087", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA6()
  bc = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bc == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 1 then
    HA111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 2 then
    HA222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 3 then
    HA333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 4 then
    HA444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 5 then
    HA555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 6 then
    HA666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bc == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000079", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000079", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000079", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000079", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000079", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000079", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA7()
  i = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if i == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 1 then
    H1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 2 then
    H2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 3 then
    H3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 4 then
    H4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 5 then
    H5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 6 then
    H6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if i == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000071", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000071", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000071", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000071", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000071", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA8()
  l = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if l == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 1 then
    H11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 2 then
    H22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 3 then
    H33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 4 then
    H44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 5 then
    H55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 6 then
    H66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if l == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000074", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000074", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000074", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000074", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000074", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000074", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA9()
  az = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if az == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 1 then
    H111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 2 then
    H222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 3 then
    H333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 4 then
    H444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 5 then
    H555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 6 then
    H666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if az == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮??𝐳𝐂𝐲𝐫𝐮𝐬

function H666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000072", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA10()
  bz = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bz == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 1 then
    A1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 2 then
    A2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 3 then
    A3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 4 then
    A4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 5 then
    A5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 6 then
    A6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bz == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000063", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000063", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000063", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000063", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000063", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000063", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA11()
  m = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if m == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 1 then
    A11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 2 then
    A22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 3 then
    A33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 4 then
    A44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 5 then
    A55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 6 then
    A66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if m == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000081", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000081", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000081", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000081", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000081", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000081", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA12()
  bb = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bb == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 1 then
    A111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 2 then
    A222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 3 then
    A333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 4 then
    A444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 5 then
    A555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 6 then
    A666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bb == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000068", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000068", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000068", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000068", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000068", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000068", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA13()
  mm = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if mm == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 1 then
    M1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 2 then
    M2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 3 then
    M3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 4 then
    M4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 5 then
    M5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 6 then
    M6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mm == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000075", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000075", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000075", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000075", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000075", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000075", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA14()
  amh = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if amh == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 1 then
    M11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 2 then
    M22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 3 then
    M33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 4 then
    M44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 5 then
    M55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 6 then
    M66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amh == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000085", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000085", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000085", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000085", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000085", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000085", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA15()
  bad = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bad == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 1 then
    M111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 2 then
    M222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 3 then
    M333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 4 then
    M444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 5 then
    M555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 6 then
    M666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bad == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000058", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000058", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000058", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000058", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000058", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000058", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA16()
  dab = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if dab == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 1 then
    B1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 2 then
    B2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 3 then
    B3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 4 then
    B4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 5 then
    B5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 6 then
    B6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dab == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000078", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000078", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000078", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000078", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000078", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000078", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA17()
  lol = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if lol == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 1 then
    B11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 2 then
    B22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 3 then
    B33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 4 then
    B44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 5 then
    B55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 6 then
    B66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lol == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000060", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000060", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000060", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000060", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000060", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000060", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA18()
  cba = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if cba == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 1 then
    B111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 2 then
    B222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 3 then
    B333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 4 then
    B444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 5 then
    B555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 6 then
    B666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cba == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000093", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000093", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000093", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000093", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000093", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000093", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA19()
  n = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if n == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 1 then
    D1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 2 then
    D2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 3 then
    D3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 4 then
    D4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 5 then
    D5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 6 then
    D6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if n == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000069", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000069", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000069", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000069", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000069", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000069", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA20()
  o = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if o == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 1 then
    D11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 2 then
    D22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 3 then
    D33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 4 then
    D44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 5 then
    D55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 6 then
    D66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if o == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000066", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000066", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000066", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000066", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000066", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000066", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA21()
  za = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if za == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 1 then
    D111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 2 then
    D222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 3 then
    D333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 4 then
    D444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 5 then
    D555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 6 then
    D666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if za == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000064", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000064", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000064", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000064", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000064", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000064", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA22()
  yy = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if yy == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 1 then
    AA1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 2 then
    AA2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 3 then
    AA3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 4 then
    AA4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 5 then
    AA5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 6 then
    AA6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yy == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000067", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000067", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000067", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000067", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000067", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000067", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA23()
  nn = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if nn == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 1 then
    AA11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 2 then
    AA22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 3 then
    AA33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 4 then
    AA44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 5 then
    AA55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 6 then
    AA66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nn == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000084", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000084", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000084", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000084", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000084", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000084", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA24()
  cc = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if cc == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 1 then
    AA111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 2 then
    AA222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 3 then
    AA333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 4 then
    AA444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 5 then
    AA555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 6 then
    AA666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cc == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000016", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000016", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000016", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000016", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000016", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000016", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA25()
  hh = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if hh == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 1 then
    HH11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 2 then
    HH22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 3 then
    HH33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 4 then
    HH44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 5 then
    HH55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 6 then
    HH66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hh == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000012", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000012", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000012", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000012", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000012", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000012", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA26()
  oo = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if oo == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 1 then
    HH111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 2 then
    HH222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 3 then
    HH333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 4 then
    HH444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 5 then
    HH555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 6 then
    HH666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oo == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000011", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000011", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000011", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000011", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000011", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000011", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA27()
  a = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if a == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 1 then
    HH1111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 2 then
    HH2222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 3 then
    HH3333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 4 then
    HH4444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 5 then
    HH5555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 6 then
    HH6666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if a == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH1111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000015", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH2222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000015", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH3333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000015", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH4444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000015", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH5555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000015", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH6666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000015", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA28()
  obb = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if obb == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 1 then
    OBB1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 2 then
    OBB2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 3 then
    OBB3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 4 then
    OBB4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 5 then
    OBB5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 6 then
    OBB6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obb == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000017", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000017", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000017", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000017", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000017", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000017", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA29()
  fo = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if fo == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 1 then
    OBB11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 2 then
    OBB22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 3 then
    OBB33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 4 then
    OBB44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 5 then
    OBB55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 6 then
    OBB66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fo == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000024", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000024", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000024", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000024", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000024", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000024", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA30()
  fire = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if fire == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 1 then
    OBB111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 2 then
    OBB222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 3 then
    OBB333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 4 then
    OBB444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 5 then
    OBB555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 6 then
    OBB666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fire == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000025", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000025", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000025", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000025", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000025", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000025", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA31()
  free = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if free == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 1 then
    FOR1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 2 then
    FOR2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 3 then
    FOR3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 4 then
    FOR4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 5 then
    FOR5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 6 then
    FOR6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if free == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000096", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000096", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000096", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000096", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000096", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000096", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA32()
  ni = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if ni == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 1 then
    FOR11()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 2 then
    FOR22()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 3 then
    FOR33()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 4 then
    FOR44()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 5 then
    FOR55()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 6 then
    FOR66()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ni == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR11()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000094", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR22()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000094", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR33()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000094", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR44()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000094", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR55()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000094", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR66()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000094", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA33()
  me = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if me == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 1 then
    FOR111()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 2 then
    FOR222()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 3 then
    FOR333()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 4 then
    FOR444()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 5 then
    FOR555()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 6 then
    FOR666()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if me == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR111()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000092", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR222()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000092", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR333()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000092", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR444()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000092", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR555()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000092", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR666()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000092", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA34()
  you = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if you == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 1 then
    ME1()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 2 then
    ME2()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 3 then
    ME3()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 4 then
    ME4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 5 then
    ME5()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 6 then
    ME6()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if you == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME1()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000005", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000091", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME2()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000029", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000091", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME3()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000002", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000091", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME4()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000059", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000091", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME5()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000001", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000091", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000045", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000091", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬




function HMA0()
  vipX = gg.choice({
    "ɢᴏɢᴏ",
    "ʀᴀᴍᴘᴀɢᴇ",
    "ᴅᴀɴᴄᴇ ᴡɪᴛʜ ᴅᴏɢ",
    "ᴛʜʀᴏɴᴇ",
    "ғғᴄᴜᴘ",
    "ʙᴏx",
    "ᴍᴀᴋᴇ ᴀʟʟ ᴅᴀɴᴄᴇ",
    "ᴄᴏʙʀᴀ2",
    "ᴄᴏʙʀᴀ3",
    "ᴇᴠᴏ ᴀᴋ",
    "ᴇᴠᴏ ᴍ1014",
    "ᴇᴠᴏ sᴄᴀʀ",
    "ᴇᴠᴏ ᴍᴘ40",
    "ᴇᴠᴏ xᴍ8",
    "ʙᴏᴏʏᴀʜ1",
    "ʙᴏᴏʏᴀʜ2",
    "ʙᴏᴏʏᴀʜ3",
    "ʙᴏᴏʏᴀʜ4",
    "ʀᴏɴᴀʟᴅᴏ 1",
    "ʀᴏɴᴀʟᴅᴏ 2",
    "sᴀɪᴛᴀᴍᴀ 1",
    "sᴀɪᴛᴀᴍᴀ2",
    "ᴢᴏᴍʙɪ 1",
    "ᴢᴏᴍʙɪ 2",
    "ᴛʀᴀᴄᴛɪᴏɴ",
    "ᴇɢʏᴘᴛ",
    "ᴅʀᴀɢᴏɴ",
    "ᴊᴀɢᴜᴀʀ",
    "ʙᴀᴛᴛᴇʟ ᴅᴀɴᴄᴇ",
    "ɢɪᴠᴇ ᴍᴇ ʏᴏᴜʀ ᴘᴀʟᴍ",
    "ʀᴏᴄᴋ",
    "ʜᴀᴄᴋᴇʀ",
    "ᴅᴀɴᴄᴇ",
    "xxxxxxx",
    "      BACK "
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if vipX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 1 then
    HMA1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 2 then
    HMA2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 3 then
    HMA3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 4 then
    HMA4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 5 then
    HMA5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 6 then
    HMA6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 7 then
    HMA7X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 8 then
    HMA8X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 9 then
    HMA9X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 10 then
    HMA10X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 11 then
    HMA11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 12 then
    HMA12X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 13 then
    HMA13X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 14 then
    HMA14X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 15 then
    HMA15X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 16 then
    HMA16X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 17 then
    HMA17X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 18 then
    HMA18X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 19 then
    HMA19X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 20 then
    HMA20X()
  end--𝐃𝐞𝐜 𝐛?? 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 21 then
    HMA21X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 22 then
    HMA22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 23 then
    HMA23X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 24 then
    HMA24X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 25 then
    HMA25X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 26 then
    HMA26X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 27 then
    HMA27X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 28 then
    HMA28X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 29 then
    HMA29X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 30 then
    HMA30X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 31 then
    HMA31X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 32 then
    HMA32X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 33 then
    HMA33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 34 then
    HMA34X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vipX == 35 then
    START()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA1X()
  vix = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if vix == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 1 then
    HMA11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 2 then
    HMA22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 3 then
    HMA33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 4 then
    HMA44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 5 then
    HMA55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 6 then
    HMA66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vix == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000089", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000089", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000089", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000089", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000089", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000089", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA2X()
  vx = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if vx == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 1 then
    HMA111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 2 then
    HMA222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 3 then
    HMA333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 4 then
    HMA444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 5 then
    HMA555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 6 then
    HMA666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if vx == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000086", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000086", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000086", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000086", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000086", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000086", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA3X()
  ax = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if ax == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 1 then
    HMA1111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 2 then
    HMA2222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 3 then
    HMA3333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 4 then
    HMA4444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 5 then
    HMA5555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 6 then
    HMA6666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ax == 7 then
    HMA()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA1111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000052", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA2222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000052", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA3333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000052", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA4444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000052", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA5555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000052", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA6666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000052", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA4X()
  abX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if abX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 1 then
    HA1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 2 then
    HA2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 3 then
    HA3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 4 then
    HA4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 5 then
    HA5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 6 then
    HA6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000014", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000014", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000014", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000014", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000014", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000014", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA5X()
  abcX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if abcX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 1 then
    HA11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 2 then
    HA22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 3 then
    HA33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 4 then
    HA44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 5 then
    HA55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 6 then
    HA66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if abcX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000087", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000087", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000087", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000087", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000087", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000087", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA6X()
  bcX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bcX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 1 then
    HA111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 2 then
    HA222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 3 then
    HA333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 4 then
    HA444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 5 then
    HA555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 6 then
    HA666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bcX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000079", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000079", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000079", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000079", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000079", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HA666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000079", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA7X()
  iX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if iX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 1 then
    H1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 2 then
    H2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 3 then
    H3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 4 then
    H4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 5 then
    H5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 6 then
    H6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if iX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000071", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000071", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000071", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000071", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000071", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA8X()
  lX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if lX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 1 then
    H11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 2 then
    H22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 3 then
    H33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 4 then
    H44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 5 then
    H55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 6 then
    H66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000074", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000074", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000074", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000074", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000074", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000074", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA9X()
  azX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if azX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 1 then
    H111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 2 then
    H222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 3 then
    H333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 4 then
    H444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 5 then
    H555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 6 then
    H666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if azX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function H666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000072", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA10X()
  bzX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bzX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 1 then
    A1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 2 then
    A2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 3 then
    A3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 4 then
    A4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 5 then
    A5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 6 then
    A6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bzX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA11X()
  mX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if mX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 1 then
    A11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 2 then
    A22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 3 then
    A33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 4 then
    A44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 5 then
    A55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 6 then
    A66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000081", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000081", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000081", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000081", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000081", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000081", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA12X()
  bbX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if bbX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 1 then
    A111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 2 then
    A222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 3 then
    A333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 4 then
    A444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 5 then
    A555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 6 then
    A666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if bbX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000068", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000068", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000068", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000068", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000068", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function A666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000068", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA13X()
  mmX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if mmX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 1 then
    M1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 2 then
    M2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 3 then
    M3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 4 then
    M4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 5 then
    M5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 6 then
    M6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if mmX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000075", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000075", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000075", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000075", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000075", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000075", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA14X()
  amhX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if amhX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 1 then
    M11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 2 then
    M22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 3 then
    M33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 4 then
    M44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 5 then
    M55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 6 then
    M66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if amhX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000085", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000085", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000085", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000085", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000085", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000085", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA15X()
  badX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if badX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 1 then
    M111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 2 then
    M222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 3 then
    M333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 4 then
    M444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 5 then
    M555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 6 then
    M666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if badX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000058", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000058", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000058", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000058", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000058", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function M666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000058", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA16X()
  dabX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if dabX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 1 then
    B1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 2 then
    B2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 3 then
    B3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 4 then
    B4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 5 then
    B5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 6 then
    B6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if dabX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000078", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000078", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000078", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000078", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000078", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000078", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA17X()
  lolX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if lolX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 1 then
    B11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 2 then
    B22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 3 then
    B33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 4 then
    B44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 5 then
    B55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 6 then
    B66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if lolX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000060", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000060", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000060", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000060", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000060", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000060", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA18X()
  cbaX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if cbaX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 1 then
    B111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 2 then
    B222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 3 then
    B333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 4 then
    B444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 5 then
    B555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 6 then
    B666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if cbaX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000093", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000093", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000093", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000063", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function B666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000093", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA19X()
  nX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if nX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 1 then
    D1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 2 then
    D2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 3 then
    D3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 4 then
    D4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 5 then
    D5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 6 then
    D6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000069", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000069", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000069", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000069", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000069", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000069", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA20X()
  oX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if oX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 1 then
    D11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 2 then
    D22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 3 then
    D33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 4 then
    D44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 5 then
    D55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 6 then
    D66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if oX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000066", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000066", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000066", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000066", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳??𝐲𝐫𝐮𝐬

function D55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000066", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000066", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA21X()
  zaX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if zaX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 1 then
    D111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 2 then
    D222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 3 then
    D333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 4 then
    D444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 5 then
    D555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 6 then
    D666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if zaX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function D666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA22W()
  yyX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if yyX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 1 then
    AA1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 2 then
    AA2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 3 then
    AA3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 4 then
    AA4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 5 then
    AA5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 6 then
    AA6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if yyX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000067", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000067", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000067", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000067", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000067", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA6()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000067", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA23X()
  nnX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if nnX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 1 then
    AA11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 2 then
    AA22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 3 then
    AA33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 4 then
    AA44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 5 then
    AA55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 6 then
    AA66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if nnX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000084", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000084", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000084", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000084", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000084", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000084", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA24X()
  ccX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if ccX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 1 then
    AA111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 2 then
    AA222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 3 then
    AA333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 4 then
    AA444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 5 then
    AA555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 6 then
    AA666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ccX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000016", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000016", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000016", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000016", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000016", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function AA666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000016", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA25X()
  hhX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if hhX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 1 then
    HH11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 2 then
    HH22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖??𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 3 then
    HH33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 4 then
    HH44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 5 then
    HH55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 6 then
    HH66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if hhX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000012", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000012", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000012", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000012", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000012", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000012", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA26X()
  ooX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if ooX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 1 then
    HH111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 2 then
    HH222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 3 then
    HH333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 4 then
    HH444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 5 then
    HH555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 6 then
    HH666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if ooX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000011", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000011", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000011", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000011", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000011", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000011", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA27X()
  aX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if aX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 1 then
    HH1111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 2 then
    HH2222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 3 then
    HH3333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 4 then
    HH4444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 5 then
    HH5555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 6 then
    HH6666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if aX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH1111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000015", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH2222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000015", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH3333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000015", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH4444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000015", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH5555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000015", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HH6666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000015", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA28X()
  obbX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if obbX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 1 then
    OBB1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 2 then
    OBB2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 3 then
    OBB3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 4 then
    OBBX4()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 5 then
    OBB5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 6 then
    OBB6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if obbX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000017", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000017", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000017", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000017", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000017", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000017", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA29X()
  foX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if foX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 1 then
    OBB11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 2 then
    OBB22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 3 then
    OBB33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 4 then
    OBB44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 5 then
    OBB55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 6 then
    OBB66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if foX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000024", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000024", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000024", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000024", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000024", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000024", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA30X()
  fireX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if fireX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 1 then
    OBB111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 2 then
    OBB222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 3 then
    OBB333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 4 then
    OBB444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 5 then
    OBB555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 6 then
    OBB666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if fireX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000025", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000025", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000025", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000025", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000025", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function OBB666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000025", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA31X()
  freeX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if freeX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 1 then
    FOR1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 2 then
    FOR2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 3 then
    FOR3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 4 then
    FOR4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 5 then
    FOR5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 6 then
    FOR6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if freeX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000096", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000096", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000096", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000096", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000096", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000096", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA32X()
  niX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if niX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 1 then
    FOR11X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 2 then
    FOR22X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 3 then
    FOR33X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 4 then
    FOR44X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 5 then
    FOR55X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 6 then
    FOR66X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if niX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR11X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000094", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR22X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000094", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR33X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000094", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR44X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000094", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR55X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000064", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR66X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000094", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA33X()
  meX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if meX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 1 then
    FOR111X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 2 then
    FOR222X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 3 then
    FOR333X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 4 then
    FOR444X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 5 then
    FOR555X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 6 then
    FOR666X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if meX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR111X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000092", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR222X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000092", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR333X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000092", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR444X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000092", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR555X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000092", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function FOR666X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000092", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function HMA34X()
  youX = gg.choice({
    "DAP",
    "KLA",
    "LOL",
    "FREE EMOTE",
    "HELLO",
    "LOVE",
    "BACK"
  }, nil, (os.date("─────────────────────── \n ---------->    ᴅᴇᴠᴇʟᴏᴘᴇᴜʀs :   BHISMA    <----------- \n--------->                ᴅᴀᴛᴇ: %D                   <----------\n -------->                ᴛɪᴍᴇ: %T                   <---------- \n────────────────────────")))
  if youX == nil then
  else
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 1 then
    ME1X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 2 then
    ME2X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 3 then
    ME3X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 4 then
    ME4X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 5 then
    ME5X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 6 then
    ME6X()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  if youX == 7 then
    HMA0()
  end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬
  menuk = -1
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME1X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000091", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000005", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME2X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000091", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000029", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME3X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000091", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000002", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME4X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000091", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000059", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME5X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000091", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000001", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬

function ME6X()
  gg.clearResults()
  gg.setRanges(gg.REGION_ANONYMOUS)
  gg.searchNumber("909000091", gg.TYPE_DWORD)
  gg.getResults(100000)
  gg.editAll("909000045", gg.TYPE_DWORD)
  gg.clearResults()
end--𝐃𝐞𝐜 𝐛𝐲 𝐖𝐮𝐭𝐳𝐂𝐲𝐫𝐮𝐬



function O()
gg.alert("🇳🇵BHISMA RAJ TIMSINA  🇳🇵")
revo.goURL("https://youtube.com/c/Neweranepalofficial")
gg.toast("🇳🇵BHISMA RAJ TIMSINA🇳🇵")
end

function EXIT()
print("🇳🇵BHISMA RAJ TIMSINA🇳🇵")
print("VISIT MY CHANNEL NEWERANEPAL OFFICIAL FOR ANY PROBLEM OR CONTACT ME IN WHATSAPP")
  print("SCRIPT ANTIBAN 100%")
  print("SCRIPT VIP MOD BY BHISMA RAJ TIMSINA")
  print("❤❤")
  print("LOVE YOU ALL")
os.exit()
main()
end 

while true do
if gg.isVisible(true) then
GARENTOD = 1
gg.setVisible(false)
end
if GARENTOD == 1 then
main()
end
end 
